...
const PRD_OVERLAY_RULES={
 ...
 'template-select':{title:'选择问卷模板',goal:'在我的问卷页面筛选并选择当前端模板，直接进入新增问卷流程。',fields:[['模板名称','最多 50 字，按模板名称模糊匹配。','state.templateSelectFilters.name'],['问卷类型','单选当前端有效问卷类型，默认全部。','state.templateSelectFilters.type'],['列表字段','仅展示序号、模板名称、介绍和创建时间。','surveyTemplateItems()'],['介绍','优先展示 description，其次 summary，均为空显示“—”。','templateSelectionIntro(template)'],['选择模板','点击整行或按 Enter、Space 后校验模板及类型，并进入新增问卷第一步。','selectTemplateForCreation(templateId)'],['关闭方式','关闭按钮、遮罩和 Escape 均关闭弹窗，不改变我的问卷筛选状态。','closeTemplateSelectionModal()']]},
...
function templateSelectionIntro(template){return String(template?.description||'').trim()||String(template?.summary||'').trim()||'—'}
...
const PRD_V16_PROFILES={
 ...
 templates:{
  ...
  features:[
   ...
   ['模板应用','弹窗整行选择','PC 在我的问卷打开模板弹窗，列表仅展示序号、模板名称、介绍和创建时间；点击整行后带入标题、类型、说明和题目并进入基础信息。','P0','模板源数据不变'],
