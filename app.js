...
const state={portal:null,page:'list',listView:'received',listFilters:{title:'',owner:'',type:'',status:''},allSurveyFilters:{title:'',owner:'',start:'',end:'',status:''},pagination:{},pageSizes:{},templateFilters:{name:'',type:'all'},templateSelectOpen:false,templateSelectFilters:{name:'',type:'all'},selectedTemplateId:null,createSurveyMenuOpen:false,surveyTypeKeyword:'',questions:[],questionMenu:-1,step:1,draftTitle:'',draftDescription:'',draftSurveyTypeId:'',draftSurveyTypeName:'',preserveSurveyTypeSnapshot:false,editingSurveyId:null,recipients:new Map(),recipientTab:'',recipientReturnPage:'',recipientExpansion:new Map(),recipientSearch:'',recipientSchoolType:'',externalRecipient:false,frequency:'single',startDate:collectionDefaultDate(),endDate:collectionDefaultDate(7),allowModify:false,dailyReminder:false,skipNonWorkday:false,reminderTime:'',channel:'internal',responseSelection:{},openQuestion:1,pcStatsQuestion:0,textAnswerViews:{},statsBack:'#all-surveys',surveyDetailBack:'#all-surveys',statsDateBySurvey:{},preview:'pc',mobileFilter:'all',mobileQuestionPicker:false,mobileSettingsSheet:'',mobileTemplateType:'all',mobileTemplateTypeOpen:false,templateTypeFilter:'all',editingSurveyTemplateId:null,mobileAnswers:{},mobileWidgetReturn:null,resourceCenterEnabled:true,attachmentSelections:{},resourceSpace:'school',resourceFolderId:'',resourceFolderName:'',resourceReturnPage:'',saveScenario:'success',saveJob:null};
...
const PRD_FIELD_GROUPS={
 ...
 surveyDetail:[
  ['问卷标题/类型/状态','只读；类型展示 surveyTypeName 历史快照，类型改名或删除后仍保持原值；问卷已删除或越权时展示不可用空态。','s.title / surveyTypeLabel(s) / s.status / s.statusText'],
  ['创建人员/时间','只读；创建时间精确到秒。','s.owner / s.createdAt'],
  ['接收人员','只读；完整展示发布时选择的人员姓名或范围名称，开启外部填写时追加“外部人员”；全部名称自动换行且不折叠。','surveyRecipientDisplayNames(s) / surveyRecipientItems(s) / s.target'],
 ...
 templates:[
  ['模板名称','筛选输入最多 50 个字符；列表名称为空显示“—”。','SURVEY_TEMPLATE_STORE[].title'],['问卷类型','模板仅保存 surveyTypeId 并动态展示当前类型名称；改名后同步显示新名称，删除后 ID 置空且显示“—”。','SURVEY_TEMPLATE_STORE[].surveyTypeId / templateSurveyTypeLabel()'],['问卷说明','选填；最多 500 个字符。','SURVEY_TEMPLATE_STORE[].description'],['模板内容','只读摘要；由前四道非空题目标题生成。','SURVEY_TEMPLATE_STORE[].summary'],['题目结构','保存题目 ID、题型、标题、必答、选项及题型专属设置。','SURVEY_TEMPLATE_STORE[].questions'],['使用模板','带入模板名称、当前有效类型、问卷说明和题目结构；类型为空时允许进入基础信息，但必须重选有效类型方可保存问卷。','loadSurveyTemplateForCreation() / draftSurveyTypeValid()'],['创建人/时间','系统模板与自定义模板均保留来源信息。','creator / createdAt / source']
 ],
 stats:[
 ...
 surveyTypes:[
  ['类型名称筛选','选填；最多 50 个字符；输入即筛选。','survey-type-keyword / data-type-name'],['类型名称','新增/编辑必填；1～20 个字符；同一端不可重名；系统预置类型不显示来源标识且不可编辑。','CONFIG[state.portal].surveyTypes[].name'],['类型说明','选填；最多 100 个字符。','surveyTypes[].description'],['关联问卷数','只读；按当前端未删除问卷的创建时类型标识动态统计。','surveyTypeSurveyCount(typeId)'],['关联模板数','只读；按当前端问卷模板的当前关联动态统计。','surveyTypeTemplateCount(typeId)'],['创建人/时间','只读；系统预置类型使用浏览器长期保存的应用首次打开时间，自定义类型使用实际新增时间。','creator / APP_FIRST_OPENED_AT / formatSystemTime()'],['操作','系统预置类型不可编辑、不可删除；自定义类型可删除且不受关联数量阻止，删除后历史问卷保留快照，关联模板类型置空。','source / edit-survey-type / delete-survey-type / removeSurveyTypeAndClearTemplates()']
 ],
 mobileAnswer:[
...
const PRD_PAGE_RULES={
 ...
 'all-surveys-detail':{title:'问卷详情',goal:'只读查看问卷配置、接收人员和答卷明细。',fields:prdFields('surveyDetail').filter(row=>!['发送/提交/未提交','完成率'].includes(row[0]))},
 ...
 'mobile-form':{title:'移动端问卷填写',goal:'填写新答卷或只读查看已经提交的问卷答卷。',fields:prdFields('mobileAnswer')},
...
const PRD_OVERLAY_RULES={
 ...
 'pc-signature':{title:'确认签字',goal:'在 PC 端完成或重新书写本次问卷签字。',fields:[['默认状态','填写页签名区域中间保持空白，右下角展示“签名”操作按钮；首次打开弹窗时画布为空。','pcSignatureInput() / data-pc-signature-open'],['签名画布','支持鼠标及 Pointer Events 书写，笔迹为黑色圆角线条。','pc-signature-canvas / openPcSignatureModal()'],['清空','仅清除弹窗中的工作副本，不立即修改本题答案。','data-pc-signature-clear'],['确认签名','画布非空时保存本次答卷签字；空白时阻止确认并提示“请完成签名”。','data-pc-signature-confirm'],['重新签名','已确认后主区域回显本次签字，右下角显示“重新签名”；再次打开时预载本题已确认签字。','data-survey-answer / data-pc-signature-open'],['关闭方式','关闭按钮、遮罩或 Escape 退出时丢弃未确认修改并保留原答案。','data-pc-signature-close / openPcSignatureModal()']]},
 'template-select':{title:'选择问卷模板',goal:'在我的问卷页面筛选并单选一个模板，确认后进入新增问卷流程。',fields:[['模板名称','最多 50 字，按模板名称模糊匹配；筛选变化后清除已选模板。','state.templateSelectFilters.name'],['问卷类型','单选当前端有效问卷类型，默认全部；已删除类型不再进入筛选项，筛选变化后清除已选模板。','state.templateSelectFilters.type'],['列表字段','仅展示序号、模板名称、问卷类型和创建时间；序号单元格内展示单选按钮与跨页连续序号。','surveyTemplateItems() / state.selectedTemplateId'],['问卷类型','按模板 surveyTypeId 动态展示当前名称；类型删除后 ID 置空并展示“—”。','templateSurveyTypeLabel(template)'],['选择模板','点击单选按钮、整行或按 Enter、Space 只更新单选状态，不进入创建页；同一时刻仅允许选中一个模板。','chooseTemplateForCreation(templateId)'],['确认选择','未选模板时按钮禁用；选中后点击确认才加载模板并进入新增问卷第一步。类型为空时提示重新选择，选择有效类型前不得保存。','confirmTemplateSelection() / selectTemplateForCreation(templateId)'],['关闭方式','取消、关闭按钮、遮罩和 Escape 均关闭弹窗并清除临时选择，不改变我的问卷筛选状态。','closeTemplateSelectionModal(method)']]},
...
function scanOverlayPrd(){
 const selectors=[['.question-picker','question-picker'],['.qr-result','qr'],['.qr-phone-shell','qr-preview'],['.stats-action-sheet','stats-actions'],['.resource-picker-modal','resource-picker'],['.save-progress-modal','save-progress'],['.template-select-modal','template-select'],['.mobile-choice-sheet','settings-choice'],['.mobile-question-sheet','question-picker'],['.stat-preview-modal','asset-preview'],['.pc-signature-modal','pc-signature'],['.mobile-template-options','template-type-picker']];
...
function switchDevice(target){
 ...
 state.templateSelectOpen=false;clearTemplateSelection();
...
function surveyTypeLabel(s){return s?.surveyTypeName||surveyTypeName(s?.surveyTypeId,'未分类')}
function templateSurveyTypeLabel(item){return validSurveyTypeId(item?.surveyTypeId)?surveyTypeName(item.surveyTypeId):'—'}
...
const SURVEY_TYPE_SNAPSHOT_OPTION='__survey_type_snapshot__';
function surveyTypeDraftOptions(){const preserve=Boolean(state.editingSurveyId&&state.preserveSurveyTypeSnapshot&&state.draftSurveyTypeName),snapshotId=String(state.draftSurveyTypeId||''),snapshot=preserve?`<option value="${SURVEY_TYPE_SNAPSHOT_OPTION}" selected>${esc(state.draftSurveyTypeName)}（原问卷类型）</option>`:'',options=currentSurveyTypes().filter(item=>!(preserve&&String(item.id)===snapshotId&&item.name===state.draftSurveyTypeName)).map(item=>`<option value="${item.id}" ${!preserve&&String(item.id)===snapshotId?'selected':''}>${esc(item.name)}</option>`).join('');return `<option value="" ${!preserve&&!snapshotId?'selected':''}>请选择问卷类型</option>${snapshot}${options}`}
function draftSurveyTypeValid(){return Boolean(state.editingSurveyId&&state.preserveSurveyTypeSnapshot&&state.draftSurveyTypeName)||validSurveyTypeId(state.draftSurveyTypeId)}
...
function publishedActions(s){
 ...
 return `${view}${statsAction}${qrAction}${statusActions}${deleteAction}`
}
...
const RECEIVED_VISIBLE_STATUSES=new Set(['running','paused','ended']);
...
function listStatusOptions(){return state.listView==='received'?[['','全部'],['running','进行中'],['paused','已暂停'],['ended','已结束']]:[['','全部'],['draft','草稿'],['not_started','未开始'],['running','进行中'],['paused','已暂停'],['ended','已结束']]}
...
function locationSelectOptions(items,selected,placeholder){const values=[...items];if(selected&&!values.includes(selected))values.unshift(selected);return `<option value="">${esc(placeholder)}</option>${values.map(item=>`<option value="${esc(item)}" ${item===selected?'selected':''}>${esc(item)}</option>`).join('')}`}
function locationDesktopInput(q,index,area){const [province='',city='',district='']=locationAreaParts(area),cities=locationProvince(province)?.cities.map(item=>item.name)||[],districts=locationCity(province,city)?.districts||[],complete=province&&city&&district?`${province}-${city}-${district}`:'';return `<div class="location-answer"><div class="location-select-row"><select data-location-province="${index}" aria-label="选择省">${locationSelectOptions(LOCATION_DIVISIONS.map(item=>item.name),province,'请选择省')}</select><select data-location-city="${index}" aria-label="选择市" ${province?'':'disabled'}>${locationSelectOptions(cities,city,'请选择市')}</select><select data-location-district="${index}" aria-label="选择区县" ${city?'':'disabled'}>${locationSelectOptions(districts,district,'请选择区县')}</select></div><input type="hidden" data-survey-answer="${index}" value="${esc(complete)}"></div>`}
function locationMobileInput(q,index,area){const complete=locationAreaParts(area).length===3?locationAreaParts(area).join('-'):'';return `<div class="location-answer location-mobile-control"><input type="hidden" data-survey-answer="${index}" value="${esc(complete)}"><button type="button" class="mobile-location-trigger ${complete?'selected':''}" data-location-picker-open="${index}"><span>${esc(complete||'请选择省市区')}</span><i aria-hidden="true">›</i></button></div>`}
function replaceLocationOptions(select,items,placeholder){select.innerHTML=locationSelectOptions(items,'',placeholder);select.disabled=!items.length}
function syncDesktopLocation(index){const province=$(`[data-location-province="${index}"]`)?.value||'',city=$(`[data-location-city="${index}"]`)?.value||'',district=$(`[data-location-district="${index}"]`)?.value||'',input=$(`[data-survey-answer="${index}"]`);if(input)input.value=province&&city&&district?[province,city,district].join('-'):''}
function openMobileLocationPicker(index){const input=$(`[data-survey-answer="${index}"]`);if(!input)return;const current=locationAreaParts(input.value),draft={province:current[0]||'',city:current[1]||'',district:current[2]||''},host=$('.device-stage')||document.body,layer=document.createElement('div');let level=draft.province?(draft.city?'district':'city'):'province';layer.className='location-picker-mask';host.appendChild(layer);const close=()=>{document.removeEventListener('keydown',onKey,true);layer.remove()},onKey=event=>{if(event.key==='Escape'){event.preventDefault();close()}},render=()=>{const levels=[['province',draft.province||'请选择省'],['city',draft.city||'请选择市'],['district',draft.district||'请选择区县']],items=level==='province'?LOCATION_DIVISIONS.map(item=>item.name):level==='city'?(locationProvince(draft.province)?.cities.map(item=>item.name)||[]):(locationCity(draft.province,draft.city)?.districts||[]),selected=draft[level];layer.innerHTML=`<section class="mobile-location-sheet" role="dialog" aria-modal="true" aria-labelledby="mobile-location-title"><header><h2 id="mobile-location-title">请选择</h2><button type="button" data-location-picker-close aria-label="关闭">×</button></header><nav class="mobile-location-tabs" aria-label="省市区选择步骤">${levels.map(([key,label],position)=>`<button type="button" class="${level===key?'active':''}" data-location-level="${key}" ${(position===1&&!draft.province)||(position===2&&!draft.city)?'disabled':''}>${esc(label)}</button>`).join('')}</nav><div class="mobile-location-list">${items.map(item=>`<button type="button" class="${item===selected?'selected':''}" data-location-option="${esc(item)}"><span>${esc(item)}</span><i aria-hidden="true">${item===selected?'✓':''}</i></button>`).join('')}</div></section>`;layer.querySelector('[data-location-picker-close]').onclick=close;layer.querySelectorAll('[data-location-level]').forEach(button=>button.onclick=()=>{if(button.disabled)return;level=button.dataset.locationLevel;render()});layer.querySelectorAll('[data-location-option]').forEach(button=>button.onclick=()=>{const value=button.dataset.locationOption;if(level==='province'){draft.province=value;draft.city='';draft.district='';level='city';render();return}if(level==='city'){draft.city=value;draft.district='';level='district';render();return}draft.district=value;input.value=[draft.province,draft.city,draft.district].join('-');const trigger=input.closest('.location-mobile-control')?.querySelector('.mobile-location-trigger');if(trigger){trigger.classList.add('selected');trigger.querySelector('span').textContent=input.value}close()})};layer.onclick=event=>{if(event.target===layer)close()};document.addEventListener('keydown',onKey,true);render()}
const MOBILE_SIGNATURE_DEMO_VALUE='已签名（演示）';
const PC_SIGNATURE_ASSETS=new Map();
const SIGNATURE_DEMO_PATH='M26 67c18 4 26-12 35-27 3 22 1 38 4 52M77 57c13-2 26-5 39-9M111 20c-3 31-2 58 3 78M87 84c25-2 49-2 73 0M168 48c9-4 18-11 24-21-1 24-4 47-1 68M191 54c13 0 25-3 36-9M197 75c12 3 24 3 36-2';
function mobileSignatureStroke(className=''){return `<svg class="mobile-signature-stroke ${className}" viewBox="0 0 260 110" aria-hidden="true"><path d="${SIGNATURE_DEMO_PATH}" fill="none" stroke="currentColor" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/></svg>`}
function signatureImageSource(value=''){const normalized=String(value||'').trim();if(!normalized)return'';if(normalized.startsWith('data:image/'))return normalized;return PC_SIGNATURE_ASSETS.get(normalized)||''}
function signatureVisual(value='',className=''){const normalized=String(value||'').trim();if(!normalized)return'';const source=signatureImageSource(normalized);return source?`<img class="signature-image ${className}" src="${esc(source)}" alt="">`:mobileSignatureStroke(`signature-render ${className}`)}
function pcSignatureInput(index,value='',readonly=false){const confirmed=String(value||'').trim();if(readonly)return confirmed?`<div class="pc-signature-readonly" aria-label="签名内容">${signatureVisual(confirmed,'pc-signature-readonly-visual')}</div>`:'<p class="readonly-answer">—</p>';const stateClass=confirmed?'confirmed':'empty',actionText=confirmed?'重新签名':'签名';return `<div class="pc-signature-answer ${stateClass}" data-pc-signature-answer="${index}"><input type="hidden" data-survey-answer="${index}" value="${esc(confirmed)}"><div class="pc-signature-trigger"><span class="pc-signature-preview" aria-label="${confirmed?'已完成签名':'签名区域为空'}">${signatureVisual(confirmed,'pc-signature-main-visual')}</span><button type="button" class="pc-signature-action" data-pc-signature-open="${index}" data-signature-preset="${esc(confirmed)}" aria-label="${actionText}">${actionText}</button></div></div>`}
function responseAnswerMarkup(question,value=''){const readable=readableAnswerValue(value);if(question&&questionKind(question.type)==='signature')return readable?`<div class="response-signature-content" aria-label="签名内容">${signatureVisual(readable,'response-signature-visual')}</div>`:'<p>—</p>';return `<p>${esc(readable||'—')}</p>`}
function signatureCanvasContext(canvas){const rect=canvas.getBoundingClientRect(),width=Math.max(1,rect.width),height=Math.max(1,rect.height),ratio=Math.max(1,Math.min(window.devicePixelRatio||1,2));canvas.width=Math.round(width*ratio);canvas.height=Math.round(height*ratio);const context=canvas.getContext('2d');context.setTransform(ratio,0,0,ratio,0,0);context.strokeStyle='#191919';context.fillStyle='#191919';context.lineWidth=3;context.lineCap='round';context.lineJoin='round';return{context,width,height}}
function clearSignatureCanvas(canvas,context){context.save();context.setTransform(1,0,0,1,0,0);context.clearRect(0,0,canvas.width,canvas.height);context.restore()}
function drawSignaturePreset(context,width,height){context.save();context.translate(width*.08,height*.12);context.scale(width*.78/260,height*.72/110);context.strokeStyle='#191919';context.lineWidth=6;context.lineCap='round';context.lineJoin='round';if(typeof Path2D==='function')context.stroke(new Path2D(SIGNATURE_DEMO_PATH));else{context.font='italic 44px KaiTi, STKaiti, serif';context.fillText(currentUserName(),30,76)}context.restore()}
function pcSignatureTrack(event,properties={}){window.__responseEvents=window.__responseEvents||[];window.__responseEvents.push({event,portal:state.portal,device:state.preview,page_key:currentPrdPageKey(),timestamp:formatSystemTime(),...properties})}
function openPcSignatureModal(trigger){
 const answer=trigger.closest('[data-pc-signature-answer]'),input=$('[data-survey-answer]',answer),m=$('#modal');if(!answer||!input||!m)return;
 const confirmedValue=input.value.trim(),sourceValue=confirmedValue,signatureSource=confirmedValue?'answer':'empty',modalId=`pc-signature-title-${trigger.dataset.pcSignatureOpen}`;pcSignatureTrack('pc_signature_open',{question_index:+trigger.dataset.pcSignatureOpen,signature_source:signatureSource});
 m.className='modal-mask show pc-signature-mask';m.innerHTML=`<section class="modal pc-signature-modal" role="dialog" aria-modal="true" aria-labelledby="${modalId}"><header class="pc-signature-modal-head"><h3 id="${modalId}">确认签字</h3><button type="button" class="pc-signature-close" data-pc-signature-close aria-label="关闭">×</button></header><div class="pc-signature-canvas-wrap"><canvas class="pc-signature-canvas" tabindex="0" aria-label="签名画布，请使用鼠标书写"></canvas></div><footer class="pc-signature-modal-actions"><button type="button" class="btn pc-signature-clear" data-pc-signature-clear>清空</button><button type="button" class="btn green" data-pc-signature-confirm>确认签名</button></footer></section>`;
 const modalElement=$('.pc-signature-modal',m),canvas=$('.pc-signature-canvas',m),confirm=$('[data-pc-signature-confirm]',m),clear=$('[data-pc-signature-clear]',m),closeButton=$('[data-pc-signature-close]',m),{context,width,height}=signatureCanvasContext(canvas);decorateOverlay(modalElement,'pc-signature');let drawing=false,hasInk=Boolean(sourceValue),ready=true,lastPoint=null;
 const reset=()=>{clearSignatureCanvas(canvas,context);hasInk=false};
 const finishClose=(method='button')=>{if(method!=='confirm')pcSignatureTrack('pc_signature_cancel',{question_index:+trigger.dataset.pcSignatureOpen,signature_source:signatureSource,close_method:method});document.removeEventListener('keydown',onKey,true);m.className='modal-mask';m.innerHTML='';trigger.focus()};
 const onKey=event=>{if(event.key==='Escape'){if($('#prd-layer')?.classList.contains('show'))return;event.preventDefault();finishClose('escape');return}if(event.key!=='Tab')return;const focusable=$$('button:not([disabled]),canvas[tabindex="0"]',modalElement);if(!focusable.length)return;const first=focusable[0],last=focusable[focusable.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}};
 const imageSource=signatureImageSource(sourceValue);if(imageSource){ready=false;confirm.disabled=true;const image=new Image();image.onload=()=>{const scale=Math.min(width*.84/image.width,height*.78/image.height),drawWidth=image.width*scale,drawHeight=image.height*scale;context.drawImage(image,(width-drawWidth)/2,(height-drawHeight)/2,drawWidth,drawHeight);ready=true;confirm.disabled=false};image.onerror=()=>{reset();ready=true;confirm.disabled=false};image.src=imageSource}else if(sourceValue)drawSignaturePreset(context,width,height);
 const point=event=>{const rect=canvas.getBoundingClientRect();return{x:event.clientX-rect.left,y:event.clientY-rect.top}};
 canvas.onpointerdown=event=>{if(!ready||(event.pointerType==='mouse'&&event.button!==0))return;event.preventDefault();drawing=true;lastPoint=point(event);canvas.setPointerCapture?.(event.pointerId);context.beginPath();context.moveTo(lastPoint.x,lastPoint.y)};
 canvas.onpointermove=event=>{if(!drawing)return;event.preventDefault();const next=point(event);context.lineTo(next.x,next.y);context.stroke();lastPoint=next;hasInk=true};
 const stopDrawing=event=>{if(!drawing)return;event.preventDefault();if(!hasInk&&lastPoint){context.beginPath();context.arc(lastPoint.x,lastPoint.y,1.6,0,Math.PI*2);context.fill();hasInk=true}drawing=false;lastPoint=null;canvas.releasePointerCapture?.(event.pointerId)};
 canvas.onpointerup=stopDrawing;canvas.onpointercancel=stopDrawing;
 clear.onclick=()=>{reset();pcSignatureTrack('pc_signature_clear',{question_index:+trigger.dataset.pcSignatureOpen,signature_source:signatureSource});canvas.focus()};confirm.onclick=()=>{if(!ready)return;if(!hasInk){toast('请完成签名');canvas.focus();return}const token=`pc-signature:${Date.now()}:${trigger.dataset.pcSignatureOpen}`;PC_SIGNATURE_ASSETS.set(token,canvas.toDataURL('image/png'));input.value=token;answer.classList.remove('empty');answer.classList.add('confirmed');trigger.dataset.signaturePreset=token;trigger.setAttribute('aria-label','重新签名');trigger.textContent='重新签名';const preview=$('.pc-signature-preview',answer);preview.innerHTML=signatureVisual(token,'pc-signature-main-visual');preview.setAttribute('aria-label','已完成签名');pcSignatureTrack('pc_signature_confirm',{question_index:+trigger.dataset.pcSignatureOpen,signature_source:signatureSource});finishClose('confirm')};
 closeButton.onclick=()=>finishClose('button');m.onclick=event=>{if(event.target===m)finishClose('mask')};document.addEventListener('keydown',onKey,true);canvas.focus();
}
function bindPcSignatureControls(root=document){$$('[data-pc-signature-open]',root).forEach(button=>button.onclick=()=>openPcSignatureModal(button))}
function mobileSignatureInput(index,value='',readonly=false){const signed=Boolean(String(value||'').trim());if(readonly&&!signed)return'<p class="readonly-answer">—</p>';return `<div class="mobile-signature-answer ${signed?'signed':''} ${readonly?'readonly':''}" data-mobile-signature-answer="${index}"><input type="hidden" data-survey-answer="${index}" value="${esc(signed?String(value):'')}"><button type="button" class="mobile-signature-empty" data-mobile-signature-open="${index}" ${readonly?'disabled':''}>点击签名</button><div class="mobile-signature-result" aria-label="已完成签名">${mobileSignatureStroke('mobile-signature-thumbnail')}${readonly?'':`<button type="button" class="mobile-signature-remove" data-mobile-signature-clear="${index}" aria-label="删除签名">×</button>`}</div></div>`}
function openMobileSignatureDemo(trigger){const answer=trigger.closest('[data-mobile-signature-answer]'),input=$('[data-survey-answer]',answer);if(!answer||!input)return;const host=trigger.closest('.qr-phone-shell')||trigger.closest('.device-stage')||$('.device-stage')||document.body,scrollRoot=trigger.closest('.qr-phone-screen')||trigger.closest('.content')||host,rect=host.getBoundingClientRect(),panelWidth=Math.max(460,Math.min(720,rect.height-82)),panelHeight=Math.max(250,Math.min(330,rect.width-64)),layer=document.createElement('div');layer.className='mobile-signature-mask';layer.innerHTML=`<section class="mobile-signature-landscape" style="width:${panelWidth}px;height:${panelHeight}px" role="dialog" aria-modal="true" aria-label="签名"><aside class="mobile-signature-tools"><i aria-label="黑色笔迹"></i>${['签字','预览','保存','擦除'].map((label,index)=>`<button type="button" class="${index===0?'active':''}" data-signature-demo-tool="${label}" aria-pressed="${index===0?'true':'false'}">${label}</button>`).join('')}</aside><div class="mobile-signature-pad"><span>请签名</span>${mobileSignatureStroke('mobile-signature-pad-stroke')}<small>演示签名</small></div><button type="button" class="mobile-signature-complete" data-mobile-signature-complete>完成</button></section>`;host.appendChild(layer);host.classList.add('mobile-signature-open');scrollRoot.classList.add('mobile-signature-scroll-lock');let hasStroke=true;const close=()=>{document.removeEventListener('keydown',onKey,true);host.classList.remove('mobile-signature-open');scrollRoot.classList.remove('mobile-signature-scroll-lock');layer.remove()},onKey=event=>{if(event.key==='Escape'){event.preventDefault();close()}};$$('[data-signature-demo-tool]',layer).forEach(button=>button.onclick=()=>{$$('[data-signature-demo-tool]',layer).forEach(item=>{item.classList.toggle('active',item===button);item.setAttribute('aria-pressed',String(item===button))});const stroke=$('.mobile-signature-pad-stroke',layer),label=button.dataset.signatureDemoTool;if(label==='擦除'){hasStroke=false;stroke?.classList.add('hidden')}else if(label==='签字'){hasStroke=true;stroke?.classList.remove('hidden')}else if(label==='预览')stroke?.classList.toggle('previewing');else if(label==='保存')$('.mobile-signature-pad small',layer).textContent='演示签名已保存'});$('[data-mobile-signature-complete]',layer).onclick=()=>{if(!hasStroke){toast('请先完成签名');return}input.value=MOBILE_SIGNATURE_DEMO_VALUE;answer.classList.add('signed');close()};layer.onclick=event=>{if(event.target===layer)close()};document.addEventListener('keydown',onKey,true)}
function bindMobileSignatureControls(root=document){$$('[data-mobile-signature-open]',root).forEach(button=>button.onclick=()=>openMobileSignatureDemo(button));$$('[data-mobile-signature-clear]',root).forEach(button=>button.onclick=()=>{const answer=button.closest('[data-mobile-signature-answer]'),input=$('[data-survey-answer]',answer);if(input)input.value='';answer?.classList.remove('signed')})}
function answerInput(q,index,value='',readonly=false,mobile=false){
  ...
  if(kind==='signature'&&mobile)return mobileSignatureInput(index,displayValue,readonly);
  if(kind==='signature')return pcSignatureInput(index,displayValue,readonly);
  if(readonly){const readonlyValue=kind==='location'?locationAreaParts(displayValue).join('-'):displayValue;return `<p class="readonly-answer">${esc(readonlyValue||'—')}</p>`}
  ...
  return mobile?locationMobileInput(q,index,displayValue):locationDesktopInput(q,index,displayValue);
}
...
function collectSurveyAnswers(s){return surveyQuestions(s).map((q,index)=>{const kind=questionKind(q.type);let value='';if(kind==='single')value=$(`input[name="answer-${index}"]:checked`)?.value||'';else if(kind==='multiple')value=$$(`input[name="answer-${index}"]:checked`).map(input=>input.value);else if(kind==='file'){const input=$(`[data-survey-answer="${index}"]`);value=Array.from(input?.files||[]).map(file=>file.name).join('、')||input?.dataset.existingAnswer||''}else value=$(`[data-survey-answer="${index}"]`)?.value.trim()||'';return[q.title,value]})}
...
function surveyRecipientDisplayNames(s){
 const names=[],seen=new Set(),snapshotPersonNames=new Map();
 (s?.recipientSnapshot||[]).forEach(item=>{const personId=item?.personId||(item?.level==='person'?item?.id:'');const name=String(item?.name||item?.nameSnapshot||'').trim();if(personId&&name)snapshotPersonNames.set(personId,name)});
 const items=[...(surveyRecipientItems(s)||[])];
 (s?.recipientRules||[]).forEach(rule=>{if(rule?.personId)return;const scopeType=rule?.scopeType||(rule?.workgroupId?'workgroup':rule?.departmentId?'department':'scope'),scopeId=rule?.scopeId||rule?.workgroupId||rule?.departmentId||rule?.schoolId||rule?.classId||rule?.parentId||rule?.id;if(!scopeId)return;items.push({...rule,id:scopeId,scopeId,scopeType,level:'scope',name:rule?.name||rule?.scopeName||rule?.nameSnapshot||rule?.departmentNameSnapshot||rule?.workgroupNameSnapshot||rule?.schoolNameSnapshot||rule?.classNameSnapshot||rule?.parentNameSnapshot||scopeId})});
 items.forEach(item=>{const personId=item?.personId||(item?.level==='person'?item?.id:''),rangeId=item?.scopeId||item?.departmentId||item?.workgroupId||item?.schoolId||item?.classId||item?.parentId||item?.id,name=String((personId&&snapshotPersonNames.get(personId))||item?.name||item?.scopeName||item?.nameSnapshot||item?.departmentNameSnapshot||item?.workgroupNameSnapshot||item?.schoolNameSnapshot||item?.classNameSnapshot||item?.parentNameSnapshot||'').trim();if(!name)return;const key=personId?`person:${personId}`:`range:${rangeId||name}`;if(seen.has(key))return;seen.add(key);names.push(name)});
 const target=String(s?.target||'').trim();
 if(!names.length&&target&&target!=='未选择')names.push(target);
 const external=s?.allowExternal===true||Boolean(s?.channel&&s.channel!=='internal');
 if(external&&!names.some(name=>name.includes('外部人员')))names.push('外部人员');
 return names.length?names:['—']
}
function allSurveyDetailPage(){const id=Number(state.page.split('/')[1]),back=state.surveyDetailBack||'#all-surveys',backLabel=back==='#list'?'返回我的问卷':'返回问卷管理',s=CONFIG[state.portal].surveys.find(x=>x.id===id&&x.deleted!==1);if(!s)return `<a class="back-link" href="${back}">← ${backLabel}</a><div class="empty-state detail-empty"><b>问卷不存在或无权查看</b><span>该问卷已删除、不属于当前组织，或链接无效。</span></div>`;const questions=surveyQuestions(s),recipientNames=surveyRecipientDisplayNames(s);return `<a class="back-link" href="${back}">← ${backLabel}</a>${pageHead('问卷详情','只读查看问卷配置、接收人员与答卷明细。')}<div class="detail-head"><div class="detail-title">${esc(s.title)}</div><div class="detail-meta"><span>创建人员：${esc(s.owner)}</span><span>创建时间：${s.createdAt}</span><span>问卷类型：${esc(surveyTypeLabel(s))}</span><span class="tag ${s.status}">${s.statusText}</span></div></div><section class="form-card"><div class="section-title">基础信息与发布设置</div><div class="detail-info-grid"><div><span>问卷名称</span><b>${esc(s.title)}</b></div><div><span>问卷类型</span><b>${esc(surveyTypeLabel(s))}</b></div><div><span>填写周期</span><b>${esc(s.time)}</b></div><div class="detail-recipients"><span>接收人员</span><b class="detail-recipient-names">${recipientNames.map(esc).join('、')}</b></div></div></section><section class="form-card readonly-questions"><div class="section-title">问卷题目 <span class="subtle">共 ${questions.length} 题</span></div>${surveyDetailQuestions(s)}</section>${s.done?`<section class="form-card"><div class="section-title">答卷明细</div>${detailStats(s)}</section>`:''}`}
function allSurveyResponsePage(){const parts=state.page.split('/'),surveyId=Number(parts[1]),responseId=decodeRoutePart(parts[3]),s=CONFIG[state.portal].surveys.find(x=>x.id===surveyId&&x.deleted!==1),back=s?`#all-surveys/${s.id}`:'#all-surveys';if(!s)return `<a class="back-link" href="#all-surveys">← 返回问卷管理</a><div class="empty-state detail-empty"><b>答卷不存在或无权查看</b><span>对应问卷已删除、不属于当前组织，或链接无效。</span></div>`;const person=statsPeopleData('submitted',s,true).find(item=>item.responseId===responseId);if(!person)return `<a class="back-link" href="${back}">← 返回问卷详情</a><div class="empty-state detail-empty"><b>答卷不存在或无权查看</b><span>该答卷不存在、已失效，或不属于当前组织。</span></div>`;const questions=statsQuestions(s);return `<a class="back-link" href="${back}">← 返回问卷详情</a>${pageHead('答卷详情','')}<section class="response-record-card"><div class="response-record-title"><div><h2>${esc(s.title)}</h2><p>答卷编号：${esc(person.responseId)}</p></div><span class="tag running">${esc(person.status)}</span></div><div class="response-record-meta"><div><span>提交人</span><b>${esc(person.name)}</b></div><div><span>所属组织</span><b>${esc(person.scope)}</b></div><div><span>身份</span><b>${esc(person.identity)}</b></div><div><span>手机号</span><b>${esc(person.mobile)}</b></div><div><span>提交时间</span><b>${esc(person.submittedAt)}</b></div></div></section><section class="response-answer response-record-answers card"><div class="response-answer-head"><div><h3>问卷内容</h3><p>共 ${questions.length} 题，以下为该用户本次提交的完整内容。</p></div></div><div class="answer-list">${questions.map((question,index)=>`<div class="answer-item"><span>${String(index+1).padStart(2,'0')}</span><div><b>${esc(question.title)}</b><small>${esc(question.type)}</small>${responseAnswerMarkup(question,person.answers[index])}</div></div>`).join('')}</div></section>`}
...
function questionValue(q={},index=0){return{id:q.id||nextQuestionId(),type:q.type||'填空',title:q.title??'',required:Boolean(q.required),options:[...(q.options||[])],imageOnly:Boolean(q.imageOnly),includeTime:Boolean(q.includeTime)}}
...
function resetCreationState(){state.step=1;state.draftTitle='';state.draftDescription='';state.draftSurveyTypeId='';state.draftSurveyTypeName='';state.preserveSurveyTypeSnapshot=false;state.editingSurveyId=null;state.questions=defaultQuestions();state.questionMenu=-1;state.questionError=-1;state.recipients.clear();state.recipientTab='';state.recipientReturnPage='';state.recipientExpansion.clear();state.recipientSearch='';state.recipientSchoolType='';state.externalRecipient=false;state.frequency='single';state.startDate=collectionDefaultDate();state.endDate=collectionDefaultDate(7);state.allowModify=false;state.dailyReminder=false;state.skipNonWorkday=false;state.reminderTime='';state.mobileSettingsSheet='';state.editingSurveyTemplateId=null;state.selectedTemplateId=null}
...
function loadSurveyTemplateContentState(item,editing=false){resetCreationState();state.editingSurveyTemplateId=editing?item.id:null;state.draftTitle=item.title||'';state.draftDescription=item.description||'';state.draftSurveyTypeId=validSurveyTypeId(item.surveyTypeId)?String(item.surveyTypeId):'';state.draftSurveyTypeName=surveyTypeName(state.draftSurveyTypeId,'');state.questions=cloneQuestions(item.questions)}
...
function openQrScanPreview(s,m){
 ...
 $$('[data-location-picker-open]',layer).forEach(button=>button.onclick=()=>openMobileLocationPicker(button.dataset.locationPickerOpen));
 bindMobileSignatureControls(layer);
...
function surveyTemplatePageData(){const f=state.templateFilters,types=currentSurveyTypes();if(f.type!=='all'&&!types.some(item=>String(item.id)===String(f.type)))f.type='all';const typeOptions=[['all','全部'],...types.map(item=>[String(item.id),item.name])],items=demoSort(surveyTemplateItems().filter(item=>String(item.title||'').includes(f.name.trim())&&(f.type==='all'||String(item.surveyTypeId)===String(f.type)))),page=paginateItems('templates',items);state.templateTypeFilter=f.type;return{f,typeOptions,page}}
...
function surveyTemplateTableMarkup(page,selecting=false){return `<div class="table-wrap template-table-wrap"><table class="data-table template-table"><thead><tr><th>序号</th><th>模板名称</th><th>问卷类型</th><th>模板内容</th><th>创建人</th><th>创建时间</th><th>操作</th></tr></thead><tbody>${page.items.length?page.items.map((x,i)=>`<tr data-template-name="${esc(x.title)}" data-template-type="${esc(x.surveyTypeId)}" data-survey-template-id="${esc(x.id)}"><td class="template-index">${page.start+i+1}</td><td><div class="title-cell">${esc(x.title)}</div></td><td>${esc(templateSurveyTypeLabel(x))}</td><td>${esc(x.summary)}</td><td>${esc(x.creator)}</td><td class="nowrap">${esc(x.createdAt)}</td><td><div class="row-actions template-row-actions"><button class="link-btn" data-action="use-template" data-template-id="${esc(x.id)}">${selecting?'使用此模板':'使用模板'}</button>${selecting||x.source==='system'?'':'<button class="link-btn" data-action="edit-survey-template">编辑</button><button class="link-btn danger" data-action="delete-survey-template">删除</button>'}</div></td></tr>`).join(''):'<tr><td colspan="7"><div class="empty-state"><b>未找到符合条件的模板</b><span>请调整筛选条件后重试</span></div></td></tr>'}</tbody></table>${paginationComponent('templates',page.totalPages,page.page,page.total)}</div>`}
function templateSelectionTrack(event,template=null,details={}){const record={event,portal_type:state.portal,device_type:state.preview,template_id:String(template?.id??state.selectedTemplateId??''),source:template?.source||'',survey_type_id:String(template?.surveyTypeId??''),timestamp:formatSystemTime(),...details};window.__templateSelectionEvents=window.__templateSelectionEvents||[];window.__templateSelectionEvents.push(record)}
function clearTemplateSelection(){state.selectedTemplateId=null}
function templateSelectionModalData(){const f=state.templateSelectFilters,types=currentSurveyTypes();if(f.type!=='all'&&!types.some(item=>String(item.id)===String(f.type)))f.type='all';const typeOptions=[['all','全部'],...types.map(item=>[String(item.id),item.name])],keyword=f.name.trim(),items=demoSort(surveyTemplateItems().filter(item=>String(item.title||'').includes(keyword)&&(f.type==='all'||String(item.surveyTypeId)===String(f.type)))),page=paginateItems('template-select-modal',items);if(state.selectedTemplateId!=null&&!page.items.some(item=>String(item.id)===String(state.selectedTemplateId)))clearTemplateSelection();return{f,typeOptions,page}}
function templateSelectionModalMarkup(){const {f,typeOptions,page}=templateSelectionModalData(),hasSelection=state.selectedTemplateId!=null;return `<div class="template-select-mask" data-template-select-mask><section class="template-select-modal" role="dialog" aria-modal="true" aria-labelledby="template-select-title"><header class="template-select-head"><div><h2 id="template-select-title">选择问卷模板</h2><p>选择系统模板或自定义模板快速创建问卷</p></div><button type="button" class="template-select-close" data-template-select-close aria-label="关闭模板选择弹窗">×</button></header><div class="template-select-filters"><label><span>模板名称：</span><input id="template-select-name" type="text" maxlength="50" value="${esc(f.name)}" placeholder="请输入模板名称"></label><label><span>问卷类型：</span><select id="template-select-type" class="select">${filterOptions(typeOptions,f.type)}</select></label><em>共 ${page.total} 条记录</em></div><div class="template-select-body"><div class="table-wrap template-select-table-wrap"><table class="data-table template-select-table"><thead><tr><th class="index-col">序号</th><th>模板名称</th><th>问卷类型</th><th>创建时间</th></tr></thead><tbody>${page.items.length?page.items.map((template,index)=>{const selected=String(state.selectedTemplateId)===String(template.id);return `<tr role="row" tabindex="0" class="${selected?'selected':''}" data-template-pick="${esc(template.id)}" aria-selected="${selected}" aria-label="选择模板：${esc(template.title)}"><td class="index-col"><span class="template-select-index"><input type="radio" name="template-choice" value="${esc(template.id)}" data-template-radio aria-label="选择模板：${esc(template.title)}"${selected?' checked':''}><b>${page.start+index+1}</b></span></td><td><div class="title-cell">${esc(template.title)}</div></td><td class="template-select-intro">${esc(templateSurveyTypeLabel(template))}</td><td class="nowrap">${esc(template.createdAt)}</td></tr>`}).join(''):'<tr><td colspan="4"><div class="empty-state"><b>未找到符合条件的模板</b><span>请调整模板名称或问卷类型后重试</span></div></td></tr>'}</tbody></table>${paginationComponent('template-select-modal',page.totalPages,page.page,page.total)}</div></div><footer class="template-select-footer"><button type="button" class="btn" data-template-select-cancel>取消</button><button type="button" class="btn primary" data-template-select-confirm${hasSelection?'':' disabled'}>确认选择</button></footer></section></div>`}
function openTemplateSelectionModal(){state.createSurveyMenuOpen=false;state.templateSelectOpen=true;state.templateSelectFilters={name:'',type:'all'};clearTemplateSelection();resetPagination('template-select-modal');renderShell();setTimeout(()=>$('#template-select-name')?.focus(),0)}
function closeTemplateSelectionModal(method='button'){if(!state.templateSelectOpen)return;const hadSelection=state.selectedTemplateId!=null;templateSelectionTrack('template_select_cancel',null,{close_method:method,had_selection:hadSelection});state.templateSelectOpen=false;clearTemplateSelection();state.createSurveyMenuOpen=true;renderShell();setTimeout(()=>$('[data-create-survey-template]')?.focus(),0)}
function chooseTemplateForCreation(templateId,selectionMethod='row'){const template=surveyTemplateItems().find(item=>String(item.id)===String(templateId));if(!template){clearTemplateSelection();toast('该模板不存在或已被删除，请重新选择');renderShell();return false}state.selectedTemplateId=String(template.id);templateSelectionTrack('template_select_choose',template,{selection_method:selectionMethod});renderShell();setTimeout(()=>$$('[data-template-pick]').find(row=>String(row.dataset.templatePick)===String(template.id))?.focus(),0);return true}
function selectTemplateForCreation(templateId){const template=surveyTemplateItems().find(item=>String(item.id)===String(templateId));if(!template){clearTemplateSelection();toast('该模板不存在或已被删除，请重新选择');renderShell();return false}const needsType=!validSurveyTypeId(template.surveyTypeId);templateSelectionTrack('template_use',template,{result:'success'});loadSurveyTemplateForCreation(template);state.step=1;state.templateSelectOpen=false;clearTemplateSelection();location.hash='list/create';toast(needsType?'模板已应用，原问卷类型已删除，请重新选择':'模板已应用，请确认基础信息');return true}
function confirmTemplateSelection(){if(state.selectedTemplateId==null)return false;return selectTemplateForCreation(state.selectedTemplateId)}
if(!window.__templateSelectEscapeBound){window.__templateSelectEscapeBound=true;document.addEventListener('keydown',event=>{if(event.key!=='Escape'||!state.templateSelectOpen||$('#prd-layer')?.classList.contains('show'))return;event.preventDefault();event.stopImmediatePropagation();closeTemplateSelectionModal('escape')})}
...
function surveyTemplateSelectionPage(){if(state.preview==='pc'){if(!state.templateSelectOpen){state.templateSelectFilters={name:'',type:'all'};clearTemplateSelection();resetPagination('template-select-modal')}state.templateSelectOpen=true;state.page='list';history.replaceState(null,'','#list');return listPage()}return mobileTemplatesPage()}
...
function bind(){
  ...
  bindPcSignatureControls();
  bindMobileSignatureControls();
  $$('[data-location-province]').forEach(select=>select.onchange=()=>{const index=select.dataset.locationProvince,city=$(`[data-location-city="${index}"]`),district=$(`[data-location-district="${index}"]`),cities=locationProvince(select.value)?.cities.map(item=>item.name)||[];replaceLocationOptions(city,cities,'请选择市');replaceLocationOptions(district,[],'请选择区县');syncDesktopLocation(index)});
  $$('[data-location-city]').forEach(select=>select.onchange=()=>{const index=select.dataset.locationCity,province=$(`[data-location-province="${index}"]`)?.value||'',district=$(`[data-location-district="${index}"]`),districts=locationCity(province,select.value)?.districts||[];replaceLocationOptions(district,districts,'请选择区县');syncDesktopLocation(index)});
  $$('[data-location-district]').forEach(select=>select.onchange=()=>syncDesktopLocation(select.dataset.locationDistrict));
  $$('[data-location-picker-open]').forEach(button=>button.onclick=()=>openMobileLocationPicker(button.dataset.locationPickerOpen));
  ...
  $('[data-template-select-close]')?.addEventListener('click',event=>{event.stopPropagation();closeTemplateSelectionModal('close_icon')});
  $('[data-template-select-cancel]')?.addEventListener('click',event=>{event.stopPropagation();closeTemplateSelectionModal('cancel')});
  $('[data-template-select-confirm]')?.addEventListener('click',event=>{event.stopPropagation();confirmTemplateSelection()});
  $('[data-template-select-mask]')?.addEventListener('click',event=>{if(event.target!==event.currentTarget)return;event.stopPropagation();closeTemplateSelectionModal('mask')});
  $$('[data-template-pick]').forEach(row=>{row.onclick=event=>{event.stopPropagation();chooseTemplateForCreation(row.dataset.templatePick,event.target.closest('[data-template-radio]')?'radio':'row')};row.onkeydown=event=>{if(event.key!=='Enter'&&event.key!==' ')return;event.preventDefault();chooseTemplateForCreation(row.dataset.templatePick,event.target.closest('[data-template-radio]')?'radio_keyboard':'row_keyboard')}});
  ...
  $$('[data-mobile-template-id]').forEach(b=>b.onclick=()=>{const template=surveyTemplateItems().find(item=>String(item.id)===String(b.dataset.mobileTemplateId));if(!template)return;const needsType=!validSurveyTypeId(template.surveyTypeId);loadSurveyTemplateForCreation(template);location.hash='mobile/create';toast(needsType?'模板已应用，原问卷类型已删除，请重新选择':'模板已应用，请确认基础信息')});
...
function setActivePage(pager,page){const key=pager.dataset.pagination;if(!key)return;if(key==='template-select-modal')clearTemplateSelection();state.pagination[paginationStateKey(key)]=page;renderShell();setTimeout(()=>toast(`已切换至第 ${page} 页`),0)}
function setPageSize(pager,size){const key=pager.dataset.pagination;if(!key||!PAGE_SIZE_OPTIONS.includes(size))return;if(key==='template-select-modal')clearTemplateSelection();const stateKey=paginationStateKey(key);state.pageSizes[stateKey]=size;state.pagination[stateKey]=1;renderShell()}
...
function applyTemplateSelectionFilters(event){state.templateSelectFilters.name=$('#template-select-name')?.value||'';state.templateSelectFilters.type=$('#template-select-type')?.value||'all';clearTemplateSelection();resetPagination('template-select-modal');templateSelectionTrack('template_filter',null,{filter_type:event?.target?.id==='template-select-type'?'survey_type':'template_name',result_count:templateSelectionModalData().page.total});rerenderFilter(event?.type==='input'?`#${event.target.id}`:'')}
...
function handleAction(a,b){
 ...
 if(a==='delete-survey-type'){const id=+b.closest('tr').dataset.typeId,item=c.surveyTypes.find(x=>x.id===id);if(!item)return;if(item.source==='system')return toast('系统默认类型不可删除');const surveyCount=surveyTypeSurveyCount(id),templateCount=surveyTypeTemplateCount(id);return modal('删除问卷类型？',`删除“${esc(item.name)}”后，该类型将不再可选；${surveyCount} 份历史问卷继续显示原类型，${templateCount} 个关联模板的问卷类型将置空。`,'确认删除',()=>{if(!removeSurveyTypeAndClearTemplates(id)){toast('问卷类型状态已变化，当前不可删除');renderShell();return}renderShell();toast('问卷类型已删除')})}
 if(a==='mobile-next-settings'){captureCreationInputs();if(!state.draftTitle){toast('请输入问卷标题');return}if(state.draftTitle.length>50){toast('问卷标题不能超过 50 个字符');return}if(!draftSurveyTypeValid()){toast('请选择问卷类型');return}if(showQuestionError(firstQuestionError()))return;state.questions.forEach(q=>{if(['单选','多选'].includes(q.type))q.options=q.options.map(x=>x.trim()).filter(Boolean)});state.recipientReturnPage='#mobile/create';location.hash='mobile/recipients';return}
...
function action(a,b){
 ...
 if(a==='next'){
  const templateWorkflow=state.page==='templates/create'||state.page.startsWith('templates/edit/');
  if(state.step===1){captureCreationInputs();if(!state.draftTitle){toast(templateWorkflow?'请输入模板名称':'请输入问卷标题');return}if(state.draftTitle.length>50){toast(templateWorkflow?'模板名称不能超过 50 个字符':'问卷标题不能超过 50 个字符');return}if(!(templateWorkflow?validSurveyTypeId(state.draftSurveyTypeId):draftSurveyTypeValid())){toast('请选择问卷类型');return}}
 ...
 if(a==='use-template'){const template=surveyTemplateItems().find(item=>String(item.id)===String(b.dataset.templateId));if(!template)return;const needsType=!validSurveyTypeId(template.surveyTypeId);loadSurveyTemplateForCreation(template);state.step=1;location.hash='list/create';renderShell();toast(needsType?'模板已应用，原问卷类型已删除，请重新选择':'模板已应用，请确认基础信息');return}
...
draftValidationError=function(){
 const basicError=!state.draftTitle.trim()?'请输入问卷标题':state.draftTitle.trim().length>50?'问卷标题不能超过 50 个字符':!draftSurveyTypeValid()?'请选择问卷类型':firstQuestionError()?.message||'';
...
deviceSwitchPage=function(target){if(state.recipientPickerDraft)discardRecipientPicker('device_switch',false);if(target==='mobile'&&(state.page==='templates/create'||state.page.startsWith('templates/edit/')))return'mobile/templates';if(target==='pc'&&state.page.startsWith('mobile/recipients')){state.step=3;return'list/create'}return recipientV2BaseDeviceSwitchPage(target)};
...
PRD_FIELD_GROUPS.recipients=[
 ...
 ['新建兼容','新建过程中若载入旧范围数据，学校角色级范围不得自动扩大为整校，继续发布前须重新选择学校。','normalizeRecipientMap() / requiresReselection']
];
...
PRD_PAGE_RULES['all-surveys-detail'].goal='只读查看问卷配置、完整接收人员和答卷明细；详情不展示数据范围字段。';
...
Object.assign(PRD_FIELD_GROUPS,{
 ...
 response:[
  ...
  ['题型专属设置','图片限制映射 accept=image/*；日期根据 includeTime 使用日期或日期时间；位置必须完成省、市、区县三级联动选择。','imageOnly / includeTime'],
  ['答案采集','多选以数组语义采集并转为可读文本；文件记录文件名；位置仅按“省-市-区县”保存；PC 签名保存独立资源令牌。','collectSurveyAnswers(s) / PC_SIGNATURE_ASSETS'],
  ['PC 签名区域','未签名前中间区域保持空白，右下角固定显示“签名”操作按钮；确认后回显本次签字并显示“重新签名”。','pcSignatureInput()'],
  ['PC 签名弹窗','点击“签名”按钮打开空白大尺寸画布；清空仅处理工作副本，画布为空不得确认，关闭、遮罩和 Escape 均回滚。','openPcSignatureModal()'],
  ['PC 签名提交','只有点击确认签名后写入本次答卷；重新签名时可预载本题已确认签字。','data-survey-answer / responseAnswerError()'],
  ['移动签名','移动填写和二维码手机预览使用横屏演示签名面板；完成后展示缩略图并支持提交前删除。','mobileSignatureInput() / openMobileSignatureDemo()'],
  ['提交频次','仅提交一次最多新增一份；每日收集每个业务日最多新增一份。','canCreateResponse(s)'],
  ['提交后只读','答卷提交成功后仅支持查看；PC 签名按图片回显，PC、移动端及直接编辑地址均不得进入修改状态。','responseDetailPage() / responseAnswerMarkup() / mobileFormPage()'],
  ['提交历史','仅展示有效答卷，按提交时间倒序；保留答卷编号和提交时间。','responseRecords(s)']
 ],
 surveyDetail:[
  ['问卷标题/类型/状态','只读；类型展示 surveyTypeName 历史快照；问卷已删除、不属于当前组织或动态 ID 非法时展示不可用空态。','s.title / surveyTypeLabel(s) / s.status'],
 ...
 createEditor:[
  ...
  ['必答及专属设置','卡片内直接设置必答；文件/图片支持仅图片，日期支持时分；位置题无额外设置。','required / imageOnly / includeTime'],
 ...
 recipients:[
  ...
  ['新建兼容','新建过程中若载入旧范围数据，学校角色级范围不得静默扩大，继续发布前要求重新选择学校。','normalizeRecipientMap() / requiresReselection'],
 ...
 mobileAnswer:[
  ...
  ['日期与位置','includeTime 控制日期时间；位置题仅采集省、市、区县，移动端使用底部分步面板，PC 使用横向三级联动下拉框。','answerInput(q, index)'],
  ['签名','未签名展示虚线卡片；点击打开横屏演示面板，完成后展示缩略图，提交前可删除，提交后只读。','mobileSignatureInput() / bindMobileSignatureControls()'],
...
draftPayload=function(existing=null){const external=state.externalRecipient,total=recipientCount(),collection=normalizedCollectionSettings(state),preserve=Boolean(existing&&state.preserveSurveyTypeSnapshot&&state.draftSurveyTypeName),selectedType=surveyTypeById(state.draftSurveyTypeId),surveyTypeId=preserve?state.draftSurveyTypeId:selectedType?.id||'',surveyTypeName=preserve?state.draftSurveyTypeName:selectedType?.name||'';return{title:state.draftTitle||existing?.title||'未命名问卷',description:state.draftDescription,surveyTypeId,surveyTypeName,channel:external?'public':'internal',allowExternal:external,target:recipientSummary()||existing?.target||'未选择',time:'未发布',startDate:state.startDate,endDate:state.endDate,internalDone:0,externalDone:0,done:0,total,recipientSelections:rawRecipientSelectionEntries(),recipientManualPartialScopes:[...state.recipientManualPartialScopes],recipientSelectionSchemaVersion:2,recipientSnapshot:[],received:false,frequency:collection.frequency,allowModify:false,skipNonWorkday:collection.skipNonWorkday,dailyReminder:collection.dailyReminder,reminderTime:collection.reminderTime,questions:cloneQuestions(state.questions)}};
...
PRD_FIELD_GROUPS.recipients=[
 ...
 ['创建过程','新建步骤保存范围对象、明确人员及手动拆分标识，跨步骤返回时保持原回显语义。','recipientSelections / recipientManualPartialScopes'],
...
renderRecipientPickerPrd=function(rule,key,entryType){
 const features=[
  ...
  ['数据保存','创建态恢复','新建步骤保存 scope/person 对象及手动拆分键，跨步骤返回时恢复原选择语义。','P0','不得提前展开范围'],
...
PRD_FIELD_GROUPS.recipients=[
 ...
 ['其他范围覆盖','人员未在当前范围选中但被其他范围覆盖时，标识“已由其他范围选中”。','recipientPersonCoveredByOtherScopes()'],
 ['成员有效性','完整范围只包含 status=active 且 canReceive=true 的人员；范围不存在或有效成员为零时阻止发布。','recipientScopeMembers() / recipientScopeIsInvalid()'],
 ['创建过程','新建步骤保存范围对象与明确人员；跨步骤返回时恢复临时副本及原选择语义。','recipientSelections / draftRecipientSnapshot()'],
...
PRD_FIELD_GROUPS.recipients=[
 ...
 ['创建过程','新建步骤保存动态部门范围、明确人员及来源；跨步骤返回时保持原选择语义，搜索条件不写入问卷数据。','recipientSelections / recipientRules / explicitPersonIds'],
 ...
 ['详情接收人员','详情完整展示已选人员或范围名称；外部填写追加“外部人员”，历史缺少结构化选择时回退原接收摘要。','surveyRecipientDisplayNames() / surveyRecipientItems() / target']
];
PRD_FIELD_GROUPS.recipientPicker=[
...
renderRecipientPickerPrd=function(rule,key,entryType){
 const features=[
  ...
  ['创建过程','原语义恢复','新建步骤保存动态部门、明确人员及来源；跨步骤返回时完整回填。','P0','不得按 personId 覆盖分类'],
...
const PRD_V16_PROFILES={
 ...
 response:{
  ...
  inScope:'七种题型渲染、PC 签名空白区域与确认弹窗、本次问卷重新签名、必答校验、答案采集、提交频次、提交后只读、本人答卷历史、二维码填写实例和提交完成页。',
  outScope:'具有法律效力的电子签章、个人签字资料修改、真实文件上传、GPS 定位、答卷审核、外部人员身份认证及行政区划字典后台维护。',
  features:[
   ...
   ['位置填写','省市区分端交互','移动端点击位置字段打开底部分步面板；PC 端在答题区域横向展示省、市、区县三级联动下拉框。','P0','未完成三级选择不形成有效答案'],
   ['PC 签名','默认空白区域','PC 始终展示完整签名区域；未签名前中间保持空白，右下角固定展示“签名”操作按钮。','P0','教育局端与学校端一致'],
   ['PC 签名','确认与重新签名','点击“签名”按钮打开空白确认签字弹窗；支持 Pointer Events 书写、清空和确认。仅确认后写入本次答案，确认后回显签字并提供“重新签名”。','P0','空白画布不得确认'],
   ['移动签名','移动演示签名','系统移动填写与二维码手机预览继续展示横屏演示面板，完成后回显签名缩略图，提交前可删除。','P0','移动端既有交互不变'],
  ...
  flow:'进入填写页后加载问卷题目；PC 签名题默认展示空白区域和右下角“签名”按钮，用户点击后在空白画布中书写，只有确认签名才写入本题；已确认后可从“重新签名”进入并编辑工作副本。完成其他输入并提交后，系统依次校验访问资格、状态、频次和答案；提交成功后进入只读结果页。',
  exceptions:'未开始、暂停、结束、删除、越权或超出提交次数时禁止提交；仅图片题选择非图片后清空；省市区未完成三级选择或关闭选择面板时不覆盖原值；PC 空白画布不得确认，关闭按钮、遮罩或 Escape 不覆盖原答案；移动签名擦除后无演示笔迹时禁止完成；通过旧链接或构造参数请求修改已提交答卷时拒绝写入。',
  data:'答案按“题目标题 + 答案值”兼容保存；多选展示时连接为可读文本，文件保存文件名，位置仅按“省-市-区县”保存。PC 未签名前不写入答案，确认后本次答卷保存独立签字资源标识；原型使用内存令牌映射画布图像并兼容历史“已签名（原型/演示）”值。移动签名仍保存“已签名（演示）”标识。记录 response_open、pc_signature_open、pc_signature_clear、pc_signature_confirm、pc_signature_cancel、response_submit、response_validation_failed；【假设】公共属性包含 survey_id、response_id、question_index、signature_source、frequency、submit_date、timestamp，禁止上报签字图像和答案正文。',
 risks:'正式环境依赖问卷访问鉴权、幂等提交、文件安全扫描、签字资源存储与标准行政区划字典。服务端须拒绝所有已提交答卷更新请求；前端隐藏入口不能替代服务端限制。',
  acceptance:'教育局端与学校端 PC 未签名前中间区域均为空白，右下角均显示“签名”按钮；首次打开弹窗时画布为空，空白不得确认；完成并确认后主区域回显本次签字并显示“重新签名”；清空、关闭、遮罩和 Escape 均符合工作副本规则；提交后签字按图片只读回显且不输出资源令牌。移动端签名交互、位置题省市区规则、图片限制、日期时分和提交频次保持正确，已提交答卷无法通过旧地址或构造请求修改。'
 },
 responseDetail:{
 ...
 directory:{
  users:'具有组织问卷查看权限的教育局或学校用户，在 PC 或移动端通过“问卷管理”浏览组织问卷、问卷配置及内部人员答卷。',
  inScope:'PC 与移动端已发布问卷的组织级筛选、只读详情、完整接收人员、题目结构、内部答卷列表和答卷详情；PC 保留分页。',
 ...
 editor:{
  ...
  features:[
   ...
   ['题型设置','专属配置','文件/图片支持仅图片，日期支持时分，位置填写固定为省市区三级选择且无额外配置。','P0','完整克隆与回填'],
 ...
 templates:{
  ...
  inScope:'模板类型筛选、系统与自定义模板、PC 端两步新增编辑、基础信息、题目结构、分页，以及从“我的问卷”单选模板并确认后创建问卷。',
  ...
  features:[
   ...
   ['类型关联','动态名称与置空','模板按 surveyTypeId 动态展示当前有效类型名称；类型改名后展示新名称，类型删除后关联 ID 置空并展示“—”。','P0','不保存类型名称快照'],
   ['模板选择','序号与单选','PC 在我的问卷打开模板弹窗，列表仅展示序号、模板名称、问卷类型和创建时间；单选按钮与跨页连续序号共同显示在序号单元格。','P0','四列表头保持不变'],
   ['模板选择','确认后应用','点击单选按钮、整行或按 Enter、Space 只更新单选状态；选中后点击“确认选择”才带入标题、类型、说明和题目并进入基础信息。','P0','未选择时确认按钮禁用'],
   ['移动选择','类型弹层','移动端问卷类型单选，无效类型自动回退全部。','P1','选择后刷新模板结果']
  ],
  flow:'用户在“我的问卷”点击从模板创建后打开居中弹窗，按模板名称或问卷类型筛选。点击单选按钮、整行或按 Enter、Space 只选中模板；选中后点击“确认选择”才应用模板并进入新增问卷基础信息。取消、关闭按钮、遮罩和 Escape 关闭弹窗且不加载模板。筛选、分页或每页条数变化时清除已选项。PC 模板中心继续用于维护和使用模板，移动端继续使用原模板选择页。',
  exceptions:'未选择模板时确认按钮禁用；模板不存在或已删除时清除选择、停留弹窗并提示重新选择；关联类型为空时允许在确认后进入基础信息并提示重新选择，选择有效类型前不得保存模板或新问卷；筛选无结果展示空态且确认按钮禁用；关闭弹窗不改变我的问卷筛选状态；同名或系统模板直接编辑时阻止保存。',
  data:'模板业务结构不变，仅新增不落库的 selectedTemplateId 临时状态。旧模板中的接收人员与发布配置读取时忽略，再次保存时清除。记录 template_filter、template_select_choose、template_select_cancel、template_use、template_save、template_validation_failed；template_use 仅在确认成功进入创建页时记录。【假设】公共属性包含 portal_type、device_type、template_id、source、survey_type_id、close_method、result、timestamp，不记录搜索原文。',
  risks:'模板结构演进须保持旧字段兼容；类型删除与模板置空必须在同一事务完成；筛选或翻页后必须清除隐藏选择，防止确认不可见模板；系统模板更新不得覆盖用户自定义模板；服务端须按组织隔离。',
  acceptance:'教育局端与学校端 PC 弹窗均为单选后确认流程，选择行本身不跳转，未选择时不可确认，取消与关闭不应用模板；类型 A 改名为 B 后模板立即显示 B，使用模板创建的新问卷固化 B；删除 B 后关联模板显示“—”，确认使用后可进入基础信息但必须重选有效类型后保存；移动端模板流程保持不变。'
 },
 types:{
  users:'教育局或学校的类型维护人员，在 PC 端筛选、新增、编辑和删除自定义问卷类型。',
  inScope:'名称筛选、系统预置类型只读、自定义类型增删改、关联问卷与模板计数、删除影响提示、问卷快照保留和模板关联置空。',
  ...
  features:[
   ...
   ['自定义类型','关联数据删除','无论是否存在关联问卷或模板均允许删除；历史问卷保留原类型快照，关联模板 surveyTypeId 置空。','P0','系统类型不可删除']
  ],
  flow:'输入关键词筛选类型；新增或编辑时打开表单，校验名称和说明后保存；删除自定义类型前展示历史问卷保留与关联模板置空影响，确认后在同一事务删除类型并清空模板关联。',
  exceptions:'名称为空、超长、重名，说明超长，系统类型操作或类型已不存在时阻止提交并提示；关联数量不作为自定义类型删除阻断条件；已删除类型不得继续出现在筛选和新建选项中。',
  data:'类型保存 id、name、description、source、creator、createdAt；问卷保存独立 surveyTypeName 快照；模板仅保存可空 surveyTypeId。关联数量按当前组织未删除问卷和模板动态统计。记录 survey_type_filter、survey_type_create、survey_type_update、survey_type_delete_confirm、survey_type_delete；【假设】公共属性包含 type_id、source、related_survey_count、related_template_count、timestamp。',
  risks:'类型删除与模板关联置空必须由服务端事务保证；删除动作不得修改任何已有问卷；首次打开时间仅为原型展示字段，生产环境使用系统类型发布日期。',
  acceptance:'两端系统类型无操作入口且不可删除；有关联数据的自定义类型可删除；确认文案说明历史问卷保留和模板置空；删除后历史问卷在“全部”结果中继续显示快照名称，类型筛选不再包含该类型，关联模板显示“—”且必须重选有效类型后保存。'
 },
 auth:{
...
resource:{
 ...
 mobileList:{
  ...
  risks:'接收范围为动态组织时须在每次进入和提交前实时解析；移动缓存不得绕过状态及权限变化。',
   acceptance:'两端移动列表和移动云首页均不展示未开始问卷；到达开始日并自动转为进行中后按接收资格展示；已提交答卷只能查看且不得修改；PC“我收到的”同步隐藏未开始状态与筛选项；发布人管理、统计和二维码入口仍可展示未开始问卷；旧链接不可绕过可见状态或只读限制。'
 },
 qr:{
...
const PRD_V16_PROFILE_KEYS={
 ...
 overlay:{
  'question-picker':'editor','survey-type':'types',confirmation:'confirmation',qr:'qr','qr-preview':'response','pc-signature':'response','stats-actions':'statsActions','resource-picker':'resource','save-progress':'resource','settings-choice':'collection','asset-preview':'assetPreview','template-select':'templates','template-type-picker':'templates'
 }
};
...
function stampPrdVersion(html){let output=html.replace(/V1\.8 选择与计数口径/g,'V1.10 选择与计数口径').replace(/V1\.9 选择与计数口径/g,'V1.10 选择与计数口径').replace(/V1\.11 选择与计数口径/g,'V1.10 选择与计数口径').replace(/编辑态保存范围语义/g,'创建态保存范围语义').replace(/编辑态范围对象/g,'创建态范围对象').replace(/问卷编辑请求示例/g,'问卷创建请求示例').replace(/草稿和未开始问卷恢复/g,'新建过程跨步骤恢复').replace(/草稿与未开始问卷恢复/g,'新建过程跨步骤恢复').replace(/草稿和未开始问卷重新编辑回填完整/g,'新建过程中跨步骤返回可完整回填').replace(/草稿与未开始问卷重新编辑完整回填/g,'新建过程中跨步骤返回可完整回填');if(!output.includes('data-prd-version='))output=output.replace(/<article class="([^"]*\bprd-document\b[^"]*)">/,`<article class="$1" data-prd-version="${PRD_CURRENT_VERSION}">`);if(!output.includes('prd-version-note'))output=output.replace('</header>',`</header><div class="prd-version-note">PRD ${PRD_CURRENT_VERSION} · 更新日期 ${PRD_CURRENT_DATE}</div>`);return output}
...
function enforceSurveyReadonlyContract(){
 Object.values(CONFIG).forEach(config=>config.surveys.forEach(survey=>{survey.allowModify=false}));
 state.allowModify=false;
 if(document.documentElement?.dataset)document.documentElement.dataset.surveyManagement='readonly-v1';
}
enforceSurveyReadonlyContract();
document.addEventListener('DOMContentLoaded',enforceSurveyReadonlyContract);
