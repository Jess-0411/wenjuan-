...
const PRD_OVERLAY_RULES={
 ...
 'template-select':{title:'选择问卷模板',goal:'在我的问卷页面筛选并选择当前端模板，直接进入新增问卷流程。',fields:[['模板名称','最多 50 字，按模板名称模糊匹配。','state.templateSelectFilters.name'],['问卷类型','单选当前端有效问卷类型，默认全部；已删除类型不再进入筛选项。','state.templateSelectFilters.type'],['列表字段','仅展示序号、模板名称、问卷类型和创建时间。','surveyTemplateItems()'],['问卷类型','按模板 surveyTypeId 动态展示当前名称；类型删除后 ID 置空并展示“—”。','templateSurveyTypeLabel(template)'],['选择模板','点击整行或按 Enter、Space 后进入新增问卷第一步；类型为空时提示重新选择，选择有效类型前不得保存。','selectTemplateForCreation(templateId) / draftSurveyTypeValid()'],['关闭方式','关闭按钮、遮罩和 Escape 均关闭弹窗，不改变我的问卷筛选状态。','closeTemplateSelectionModal()']]},
...
function selectTemplateForCreation(templateId){const template=surveyTemplateItems().find(item=>String(item.id)===String(templateId));if(!template){toast('该模板不存在或已被删除，请重新选择');renderShell();return}const needsType=!validSurveyTypeId(template.surveyTypeId);loadSurveyTemplateForCreation(template);state.step=1;state.templateSelectOpen=false;location.hash='list/create';toast(needsType?'模板已应用，原问卷类型已删除，请重新选择':'模板已应用，请确认基础信息')}
...
Object.assign(PRD_FIELD_GROUPS,{
 ...
 response:[
  ...
  ['答案采集','多选以数组语义采集并转为可读文本；文件记录文件名；位置仅按“省-市-区县”保存。','collectSurveyAnswers(s)'],
...
const PRD_V16_PROFILES={
 ...
 response:{
  ...
  data:'答案按“题目标题 + 答案值”兼容保存；多选展示时连接为可读文本，文件保存文件名，位置仅按“省-市-区县”保存。历史位置答案若包含附加内容，展示和再次读取时仅保留省市区。正式环境行政区划读取统一标准字典并按版本管理。记录 response_open、response_submit、response_validation_failed；【假设】公共属性包含 survey_id、response_id、question_count、frequency、submit_date、timestamp，禁止上报答案正文。',
 risks:'正式环境依赖问卷访问鉴权、幂等提交、文件安全扫描、签名能力及标准行政区划字典。服务端必须拒绝所有已提交答卷更新请求，前端隐藏入口不能替代服务端限制。',
  acceptance:'七种题型在 PC 与移动端使用一致的数据口径；位置题只包含省、市、区县，移动端以底部面板逐级选择，PC 在答题区域横向展示三个联动下拉框；切换省或市时清空下级选项，未完整选择时不形成有效答案，历史答案仅回显省市区；必答、图片限制和日期时分规则正确；提交频次控制正确；已提交答卷在 PC 与移动端仅可查看且无法通过旧地址或构造请求修改。'
 },
 responseDetail:{
 ...
 templates:{
  ...
  exceptions:'模板不存在或已删除时阻止进入新增问卷；关联类型为空时允许进入基础信息并提示重新选择，选择有效类型前不得保存模板或新问卷；筛选无结果展示空态；关闭弹窗不改变我的问卷筛选状态；同名或系统模板直接编辑时阻止保存。',
  ...
  risks:'模板结构演进须保持旧字段兼容；类型删除与模板置空必须在同一事务完成；系统模板更新不得覆盖用户自定义模板；服务端须按组织隔离。',
  acceptance:'类型 A 改名为 B 后模板立即显示 B，使用模板创建的新问卷固化 B；删除 B 后关联模板显示“—”，编辑或使用该模板可进入基础信息但必须重选有效类型后保存；教育局端、学校端及 PC、移动端行为一致。'
 },
 types:{
...
const PRD_V16_PROFILE_KEYS={
 ...
 overlay:{
  'question-picker':'editor','survey-type':'types',confirmation:'confirmation',qr:'qr','qr-preview':'response','stats-actions':'statsActions','resource-picker':'resource','save-progress':'resource','settings-choice':'collection','asset-preview':'assetPreview','template-select':'templates','template-type-picker':'templates'
 }
};
