...
Object.assign(PRD_FIELD_GROUPS,{
 ...
 mobileAnswer:[
  ...
  ['日期与位置','includeTime 控制日期时间；移动端位置使用省、市、区县分步面板，PC 使用三级联动选择；detailedAddress 控制详细地址输入。','answerInput(q, index)'],
...
const PRD_V16_PROFILES={
 ...
 response:{
  ...
  features:[
   ...
   ['位置填写','省市区联动','移动端按省、市、区县分步选择并在区县确认后回填；PC 端使用省、市、区县三级联动下拉框。','P0','未完成三级选择不形成有效答案'],
  ...
  exceptions:'未开始、暂停、结束、删除、越权或超出提交次数时禁止提交；仅图片题选择非图片后清空；省市区未完成三级选择或关闭选择面板时不覆盖原值；签名演示取消时保持未填写；修改期间失去接收资格时拒绝保存。',
  data:'答案按“题目标题 + 答案值”兼容保存；多选展示时连接为可读文本，文件保存文件名，位置按“省-市-区县｜详细地址”保存。正式环境行政区划读取统一标准字典并按版本管理。记录 response_open、response_submit、response_validation_failed、response_update；【假设】公共属性包含 survey_id、response_id、question_count、frequency、submit_date、timestamp，禁止上报答案正文。',
 risks:'正式环境依赖问卷访问鉴权、幂等提交、文件安全扫描、签名能力及标准行政区划字典。服务端必须防止重复提交并保留修改审计记录。',
 acceptance:'七种题型在 PC 与移动端展示和采集一致；位置题在移动端可依次选择省、市、区县，PC 可完成三级联动，未完整选择时不形成有效答案，已有答案可正确回显；必答、图片限制、日期时分和详细地址规则正确；提交频次控制正确；修改原答卷不增加提交数。'
 },
 responseDetail:{
