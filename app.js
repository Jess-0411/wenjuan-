...
const CONFIG={
 bureau:{name:'教育局端',org:'临江县教育局',user:'王明 · 局管理员',menus:[['list','我的问卷'],['all-surveys','问卷管理'],['templates','模板中心'],['survey-types','问卷类型']],stats:[['问卷总数','128'],['进行中','18'],['本月有效答卷','12,680'],['待填写人数','1,246']],surveyTypes:[
  ...
  {id:1,title:'2026年全县教师数字素养调研',type:'系统定向',channel:'internal',target:'全县中小学教师',owner:'李静',time:'08-05 至 08-20',createdAt:'2026-08-05 09:18:36',deleted:0,status:'running',statusText:'进行中',done:3268,total:3820,received:true,fillStatus:'submitted',frequency:'unlimited',allowModify:true},
  ...
  {id:9,title:'2026年社会公众教育意见征集',type:'公开问卷',channel:'public',target:'社会公众及其他外部人员',owner:'王明',time:'08-12 至 08-31',createdAt:'2026-08-12 14:08:25',deleted:0,status:'running',statusText:'进行中',done:286,total:0,received:false,frequency:'unlimited',allowModify:false},
 ...
 school:{name:'学校端',org:'临江县实验学校',user:'陈芳 · 校管理员',menus:[['list','我的问卷'],['all-surveys','问卷管理'],['templates','模板中心'],['survey-types','问卷类型']],stats:[['问卷总数','64'],['进行中','8'],['本周有效答卷','2,486'],['待填写人数','286']],surveyTypes:[
  ...
  {id:21,title:'校园服务社会满意度调查',type:'公开问卷',channel:'public',target:'家长及其他外部人员',owner:'陈芳',time:'08-10 至 08-31',createdAt:'2026-08-10 10:18:35',deleted:0,status:'running',statusText:'进行中',done:324,total:0,received:false,frequency:'unlimited',allowModify:false},
...
function installDemoCoverageData(){
 ...
 const surveySeeds={
  bureau:[
   ...
   {id:32,title:'家校协同公开意见补充征集',surveyTypeId:104,channel:'public',target:'学生家长及社会公众',owner:'办公室',time:`${collectionDefaultDate(-2).slice(5)} 至 ${collectionDefaultDate(5).slice(5)}`,startDate:collectionDefaultDate(-2),endDate:collectionDefaultDate(5),createdAt:'2026-08-14 10:20:00',deleted:0,status:'running',statusText:'进行中',done:88,total:0,received:true,fillStatus:'not_started',frequency:'unlimited',allowModify:false,demoRank:3,widgetRank:22},
  ...
  school:[
   ...
   {id:42,title:'校园服务公开意见补充征集',surveyTypeId:204,channel:'public',target:'学生家长及校外访客',owner:'德育处',time:`${collectionDefaultDate(-2).slice(5)} 至 ${collectionDefaultDate(5).slice(5)}`,startDate:collectionDefaultDate(-2),endDate:collectionDefaultDate(5),createdAt:'2026-08-14 10:20:00',deleted:0,status:'running',statusText:'进行中',done:76,total:0,received:true,fillStatus:'not_started',frequency:'unlimited',allowModify:false,demoRank:3,widgetRank:22},
...
const COLLECTION_FREQUENCIES=[{value:'single',label:'仅提交一次',description:'有效期内仅能新增一份答卷'},{value:'daily',label:'每日收集一次',description:'每个自然日最多新增一份答卷'},{value:'unlimited',label:'不限次数提交',description:'有效期内可多次新增答卷'}];
...
const MODIFY_RULES=[{value:'true',label:'截止前允许修改',description:'修改所选原答卷，不增加提交次数'},{value:'false',label:'提交后不可修改',description:'提交后仅可查看本次答卷'}];
...
const PRD_FIELD_GROUPS={
 ...
 mobileAnswer:[
  ['姓名','必填；1～20 个字符；去除首尾空格。','state.mobileAnswers[surveyId].name'],['性别','必填；男、女二选一。','state.mobileAnswers[surveyId].gender'],['微信号','选填；6～20 个字符；字母开头，仅字母、数字、下划线、连字符。','state.mobileAnswers[surveyId].wechat'],['QQ号','选填；5～12 位数字。','state.mobileAnswers[surveyId].qq'],['只读状态','已提交时禁用字段；允许修改时展示修改按钮。','s.fillStatus / s.allowModify']
 ],
 mobileCreate:[
...
function scanOverlayPrd(){
 const selectors=[['.question-picker','question-picker'],['.qr-result','qr'],['.qr-phone-shell','qr-preview'],['.stats-action-sheet','stats-actions'],['.resource-picker-modal','resource-picker'],['.save-progress-modal','save-progress'],['.mobile-choice-sheet','settings-choice'],['.mobile-question-sheet','question-picker'],['.stat-preview-modal','asset-preview'],['.mobile-template-options','template-type-picker']];
...
function mobileWidgetSurveys(){return widgetSurveys()}
...
function canCreateResponse(s){syncSurveyTemporalStatus(s);if(s.status!=='running')return false;const records=responseRecords(s);if(!records.length)return true;if(s.frequency==='unlimited')return true;if(s.frequency==='daily')return !records.some(r=>r.submittedAt.slice(0,10)===beijingDateKey());return false}
...
function frequencyText(value){return ({single:'仅提交一次',daily:'每日收集一次',unlimited:'不限次数提交'})[value]||'仅提交一次'}
...
function answerInput(q,index,value='',readonly=false){
  ...
  return `<div class="prototype-answer-action"><input data-survey-answer="${index}" value="${esc(displayValue.split('｜')[0]||'')}" readonly placeholder="尚未定位"><button type="button" data-prototype-location="${index}">获取位置</button>${q.detailedAddress?`<input data-location-detail="${index}" value="${esc(displayValue.split('｜')[1]||'')}" maxlength="100" placeholder="请输入详细地址">`:''}</div>`;
}
function surveyAnswerFields(s,answers=[],readonly=false){return surveyQuestions(s).map((q,index)=>`<div class="survey-answer-field ${q.required?'required':''}"><h3><b>${String(index+1).padStart(2,'0')}.</b> ${esc(q.title)}${q.required?'<em>*</em>':''}</h3>${answerInput(q,index,answerAt(answers,index),readonly)}</div>`).join('')}
...
function responseFillPage(){const parts=state.page.split('/'),s=receivedSurveyFromRoute(),editingId=parts[3],editing=editingId?currentPersonResponseRecords(s||{}).find(r=>r.id===editingId):null;if(!s)return responseNotFound();syncSurveyTemporalStatus(s);if(editingId&&(!editing||s.status!=='running'||!s.allowModify||!surveyAllowsCurrentPerson(s)))return `${pageHead('填写问卷','')}<a class="back-link" href="#list/response/${s.id}">← 返回本人答卷</a><div class="empty-state detail-empty"><b>当前答卷不可修改</b><span>${s.status==='not_started'?'问卷尚未到开始日期，当前不可修改答卷。':!surveyAllowsCurrentPerson(s)?'您当前已不在问卷接收范围内。':'仅进行中且允许提交后修改的问卷支持修改原答卷。'}</span></div>`;if(!editing&&!canCreateResponse(s))return `${pageHead('填写问卷','')}<a class="back-link" href="#list">← 返回我的问卷</a><div class="empty-state detail-empty"><b>当前不可新增提交</b><span>${s.status==='not_started'?'问卷尚未到开始日期。':s.status==='running'&&!surveyAllowsCurrentPerson(s)?'您当前已不在问卷接收范围内。':s.status==='running'?`${frequencyText(s.frequency)}的提交次数已用完。`:'问卷已暂停或结束。'}</span></div>`;return `<a class="back-link" href="${editing?`#list/response/${s.id}`:'#list'}">← ${editing?'返回本人答卷':'返回我的问卷'}</a>${pageHead(editing?'修改本次答卷':'提交问卷','')}<div class="response-form-card card"><div class="response-form-head"><h2>${esc(s.title)}</h2><p>${editing?`正在修改 ${editing.id}，保存后不会新增提交次数。`:`${frequencyText(s.frequency)}，本次提交将生成一条新的答卷记录。`}</p></div><div class="response-form dynamic-response-form">${surveyAnswerFields(s,editing?.answers||[])}</div><div class="footer-actions"><a class="btn" href="${editing?`#list/response/${s.id}`:'#list'}">取消</a><button class="btn primary" data-action="submit-my-response" data-survey-id="${s.id}" ${editing?`data-response-id="${editing.id}"`:''}>${editing?'保存修改':'确认提交'}</button></div></div>`}
...
function questionCard(q,i){
  const preview=q.type==='填空'?'请输入':q.type==='日期'?`▣ 填写格式: 年-月-日${q.includeTime?' 时:分':''}`:q.type==='文件/图片'?'文件/图片上传区域':q.type==='签名'?'签名区域':q.type==='位置'?`⊕ 定位位置：省-市-区${q.detailedAddress?' + 详细地址':''}`:'';
...
function resetCreationState(){state.step=1;state.draftTitle='';state.draftDescription='';state.draftSurveyTypeId='';state.editingSurveyId=null;state.questions=defaultQuestions();state.questionMenu=-1;state.questionError=-1;state.recipients.clear();state.recipientTab='';state.recipientReturnPage='';state.recipientExpansion.clear();state.recipientSearch='';state.recipientSchoolType='';state.externalRecipient=false;state.frequency='single';state.startDate=collectionDefaultDate();state.endDate=collectionDefaultDate(7);state.allowModify=true;state.dailyReminder=false;state.skipNonWorkday=false;state.reminderTime='';state.mobileSettingsSheet='';state.editingSurveyTemplateId=null}
...
function surveyTemplateSelectionPage(){const {f,typeOptions,page}=surveyTemplatePageData();return `<a class="back-link" href="#list">← 返回我的问卷</a>${pageHead('选择问卷模板','选择系统模板或自定义模板快速创建问卷。')}${surveyTemplateFilterMarkup(f,typeOptions,page.total)}${surveyTemplateTableMarkup(page,true)}`}
...
function mobileFormContent(s,readonly=false,preview=false,answerOverride=null,editingId=''){const answers=answerOverride??state.mobileAnswers[s.id]??[];return `<div class="mobile-form-intro"><h1>${esc(s.title)}</h1><p>${esc(s.description||'请如实填写')}</p></div><div class="mobile-dynamic-form">${surveyAnswerFields(s,answers,readonly)}</div><div class="mobile-form-actions">${readonly?(s.status==='running'&&s.allowModify&&surveyAllowsCurrentPerson(s)?`<button class="outline" data-mobile-edit="${s.id}">修改</button>`:'')+`<a href="#stats/detail/${s.id}">查看此问卷统计 &gt;</a>`:preview?'<button data-qr-preview-submit>提交</button>':`<button data-action="mobile-submit" data-survey-id="${s.id}" ${editingId?`data-response-id="${esc(editingId)}"`:''}>${editingId?'保存修改':'提交'}</button>`}</div>`}
function mobileFormPage(id,view=false,external=false){const s=CONFIG[state.portal].surveys.find(item=>item.id===id&&item.deleted!==1),externalAllowed=Boolean(s&&external&&(s.allowExternal||s.channel==='public'));if(!s||external&&!externalAllowed||!external&&!isReceivedSurveyVisible(s))return `${statsMobileHead('填写','#mobile/fill')}<div class="mobile-unavailable"><i>!</i><b>问卷暂不可访问</b><span>该问卷尚未开始、已移除或不在您的接收范围内。</span></div>`;syncSurveyTemporalStatus(s);const records=external?[]:currentPersonResponseRecords(s),readonly=!external&&view&&s.fillStatus==='submitted',editing=!external&&!view&&state.page.endsWith('/edit')&&s.fillStatus==='submitted',record=(editing||readonly)?records[0]:null;if(editing&&(!record||!s.allowModify||!surveyAllowsCurrentPerson(s)))return `${statsMobileHead('填写','#mobile/fill')}<div class="mobile-unavailable"><i>!</i><b>当前答卷不可修改</b><span>仅进行中、允许修改且仍在接收范围内的问卷支持修改。</span></div>`;if(!readonly&&s.status!=='running')return `${statsMobileHead('填写','#mobile/fill')}<div class="mobile-unavailable"><i>!</i><b>问卷${esc(s.statusText)}</b><span>${s.status==='not_started'?`该问卷将于 ${esc(s.startDate)} 开始，请在开始后填写。`:'当前问卷暂不可填写，请联系问卷发布人。'}</span></div>`;if(!external&&!readonly&&!surveyAllowsCurrentPerson(s))return `${statsMobileHead('填写','#mobile/fill')}<div class="mobile-unavailable"><i>!</i><b>当前不可填写</b><span>您当前已不在问卷接收范围内，历史答卷仍可查看。</span></div>`;return `${statsMobileHead('填写','#mobile/fill')}${mobileFormContent(s,readonly,false,record?.answers||null,editing?record.id:'')}`}
...
function statsSummaryCards(s,snapshot,mobile=state.preview==='mobile'){
 ...
 if(mobile){
  const submitted=trackable?`<button class="submitted" data-detail-kind="submitted" data-survey-id="${s.id}"><strong>${snapshot.internalDone}</strong><span>已提交 ${submittedTip}</span><em>查看明细</em></button>`:`<div class="submitted stats-metric untrackable"><strong>0</strong><span>已提交 ${submittedTip}</span><em>暂无内部接收人员</em></div>`;
  const pending=trackable?`<button class="pending" data-detail-kind="pending" data-survey-id="${s.id}"><strong>${snapshot.pending}</strong><span>未提交 ${pendingTip}</span><em>查看明细</em></button>`:`<div class="pending stats-metric untrackable"><strong>—</strong><span>未提交 ${pendingTip}</span><em>暂无内部接收人员</em></div>`;
  const external=`<div class="external stats-metric"><strong>${snapshot.externalDone}</strong><span>外部提交数 ${externalTip}</span><em>不支持人员明细</em></div>`;
  const rate=trackable?`<div class="rate stats-summary-rate"><strong>${snapshot.rate}%</strong><span>完成率 ${rateTip}</span><em>内部提交进度</em></div>`:`<div class="rate stats-summary-rate untrackable"><strong>—</strong><span>完成率 ${rateTip}</span><em>暂无内部接收人员</em></div>`;
...
function bind(){
  ...
  $$('[data-prototype-location]').forEach(button=>button.onclick=()=>{const input=$(`[data-survey-answer="${button.dataset.prototypeLocation}"]`);if(input)input.value='浙江省-杭州市-临江县'});
  ...
  $('[data-create-survey-template]')?.addEventListener('click',event=>{event.stopPropagation();state.createSurveyMenuOpen=false;location.hash='list/template-select'});
  ...
  $$('[data-mobile-edit]').forEach(b=>b.onclick=()=>{location.hash=`mobile/fill/${b.dataset.mobileEdit}/edit`});
  $$('[data-action="mobile-submit"]').forEach(b=>b.onclick=e=>{
   e.stopPropagation();const s=mobileSurvey(+b.dataset.surveyId),external=isExternalFillContext(s),records=s?(MY_RESPONSES[state.portal][s.id]||(MY_RESPONSES[state.portal][s.id]=[])):[],editing=b.dataset.responseId?records.find(record=>record.id===b.dataset.responseId&&record.external!==true&&record.personId===currentRecipientPersonId()):null,allowed=editing?Boolean(s&&s.status==='running'&&s.allowModify&&surveyAllowsCurrentPerson(s)):canCreateResponse(s);
   if(!allowed){toast(s?.status==='not_started'?'问卷尚未开始':!external&&!surveyAllowsCurrentPerson(s)?'您当前已不在问卷接收范围内':editing?'当前答卷不可修改':'当前问卷不可提交');return}
   const answers=collectSurveyAnswers(s),error=responseAnswerError(s,answers);if(error){toast(error);return}
   modal(editing?'确认保存修改？':'确认提交？',editing?'保存后将更新原答卷，不增加提交次数。':'提交后将保存本次填写内容。',editing?'确认保存':'确认提交',()=>{
    syncSurveyTemporalStatus(s);const stillAllowed=editing?Boolean(s.deleted!==1&&s.status==='running'&&s.allowModify&&surveyAllowsCurrentPerson(s)):canCreateResponse(s);if(!stillAllowed){toast(editing?'当前答卷不可修改':'当前问卷不可提交');renderShell();return}
    state.mobileAnswers[s.id]=answers;
    if(editing){editing.answers=answers;editing.updatedAt=responseTimestamp();state.responseSelection[s.id]=editing.id;location.hash=`mobile/fill/${s.id}/view`;renderShell();toast('答卷修改已保存');return}
...
function currentMobileCloudHomeWidget(){
	 const c=CONFIG[state.portal],rows=widgetSurveys(),metrics=syncCloudWidgetStats(),key='mobile-widget',badge=s=>`<em class="tag ${s.status}">${esc(s.statusText)}</em>`;
	 return `<div class="cloud-mobile-home"><button type="button" class="cloud-mobile-return" data-mobile-widget-return>← 返回主应用</button><header><span>云平台首页 · ${c.name}</span><b>${c.org}</b></header><section class="cloud-mobile-section"><div class="cloud-mobile-title"><div><h2>问卷调研</h2><p>问卷运行统计</p></div>${prdButton(key,'page','cloud-widget-prd-trigger')}</div><div class="cloud-mobile-widget cloud-stats-only" aria-label="问卷调研统计数据"><div class="cloud-stat-grid">${metrics.map(m=>`<div><b>${m[1]}</b><span>${m[0]}</span></div>`).join('')}</div><div class="cloud-mobile-latest"><h3 class="cloud-latest-title">最新问卷</h3>${rows.length?rows.map(s=>`<a class="cloud-mobile-latest-row" href="#stats/detail/${s.id}"><b>${esc(s.title)}${badge(s)}</b><span>截止时间：${esc(mobileWidgetDeadline(s))}</span></a>`).join(''):'<div class="cloud-widget-empty">暂无问卷</div>'}</div></div></section><section class="cloud-mobile-placeholder"><h2>常用应用</h2><div></div></section></div>`
}
...
cloudWidgetRules=function(){return `
	 ...
	 <section><h3>2. 用户与使用场景</h3><p>教育局与学校授权用户在 PC 或移动云首页查看组织问卷概览与最新问卷。</p></section>
	 <section><h3>3. 需求范围</h3><h4>In Scope</h4><p>两端首页入口删除、两项统计、PC 与移动端最新五条组织问卷、截止日期、状态标签及统计详情跳转。</p><h4>Out of Scope</h4><p>移动端我的问卷列表、问卷创建表单、其他页面时间字段调整及后端接口实现。</p></section>
	 <section><h3>4. 功能需求列表</h3>${prdTable(['功能模块','功能点','需求描述','优先级（P0 / P1 / P2）','备注说明'],[['应用导航','删除首页','删除首页入口；默认进入我的问卷；旧首页地址重定向我的问卷。','P0','两端一致'],['云首页','顶部统计','仅显示问卷总数与进行中问卷，并缩小统计区高度。','P0','未开始不计入进行中'],['PC 云首页','最新五条问卷','最多展示当前组织最新五条已发布问卷，点击进入统计详情。','P0','按创建时间和 ID 倒序'],['移动云首页','最新五条问卷','最多展示当前组织最新五条已发布问卷、状态和截止时间，点击进入统计详情。','P0','未开始同步展示']])}</section>
	 <section><h3>5. 核心流程与交互说明</h3><p>PC 与移动端均展示两项统计与最新五条组织问卷；点击问卷进入统计详情，PC“查看更多”进入问卷管理。</p></section>
	 <section><h3>6. 异常场景与边界条件</h3><p>不足五条时按实际数量展示；无已发布问卷时展示“暂无问卷”。截止日期缺失展示 —；加载失败和无权限不得显示为零。</p></section>
	 <section><h3>7. 数据口径与埋点需求</h3><p>PC 与移动端列表均取当前组织未删除且已发布问卷，按创建时间和 ID 倒序最多五条。问卷总数包含未删除草稿；进行中仅计同步日期状态后 status=running 的问卷。记录 cloud_widget_open、survey_widget_entry_click、prd_rule_open、prd_rule_close；【假设】公共属性包含 portal、device、survey_id、entry_target 和 timestamp。</p></section>
	 <section><h3>8. 风险、依赖与限制</h3><p>当前为前端演示数据；正式统计依赖组织权限、个人答卷状态和聚合接口，缓存必须隔离组织、用户及权限范围。生产数据使用独立 deadline 字段，原型兼容已有时间文本。</p></section>
	 <section><h3>9. 验收标准</h3><p>两端无首页菜单，旧地址进入我的问卷；PC 与移动端均仅显示问卷总数和进行中问卷；两端最新问卷不超过五条并进入统计详情；未开始状态、空态和截止日期展示正确。</p></section>
`}

/* PRD V1.9: keep every page-level document aligned with the current prototype. */
const PRD_CURRENT_VERSION='V1.9';
const PRD_CURRENT_DATE='2026-09-07';
Object.assign(PRD_FIELD_GROUPS,{
 surveyList:[
  ...
  ['我收到的操作','根据有效答卷、填写频次、问卷状态及修改权限展示查看、提交问卷或修改答卷。','receivedActions(s) / canCreateResponse(s)']
 ],
 response:[
  ...
  ['题型专属设置','图片限制映射 accept=image/*；日期根据 includeTime 使用日期或日期时间；位置根据 detailedAddress 展示详细地址。','imageOnly / includeTime / detailedAddress'],
  ['答案采集','多选以数组语义采集并转为可读文本；文件记录文件名；位置记录区域及可选详细地址。','collectSurveyAnswers(s)'],
  ['提交频次','仅提交一次最多新增一份；每日收集每个业务日最多新增一份；不限次数可重复提交。','canCreateResponse(s)'],
  ['答卷修改','仅进行中且 allowModify=true 时可修改原答卷；保存修改不得新增提交次数。','responseRecords(s) / allowModify'],
 ...
 publish:[
  ...
  ['收集方式','支持仅提交一次、每日收集一次、不限次数提交；切换方式时统一规范化每日配置。','state.frequency / normalizedCollectionSettings()'],
 ...
 mobileAnswer:[
  ...
  ['日期与位置','includeTime 控制日期时间；detailedAddress 控制详细地址输入。','answerInput(q, index)'],
...
const PRD_V16_PROFILES={
 ...
 response:{
  users:'问卷接收人或通过有效二维码进入的外部填写人，在 PC 或移动端填写、提交、查看及按权限修改答卷。',
  inScope:'七种题型渲染、必答校验、答案采集、提交频次、本人答卷历史、原答卷修改、二维码填写实例和提交完成页。',
  outScope:'真实文件上传、真实电子签章、真实定位权限、答卷审核和外部人员身份认证。',
  features:[
   ...
   ['提交频次','次数控制','单次问卷最多新增一份；每日问卷每个业务日最多一份；不限次数问卷可重复提交。','P0','以业务时区自然日判断'],
   ['答卷修改','原答卷覆盖','仅进行中且允许修改时展示入口；保存修改覆盖原答卷，不新增提交次数。','P0','服务端再次鉴权'],
   ['答卷历史','有效记录','本人答卷仅展示有效记录，按提交时间倒序，并保留答卷编号。','P1','无记录展示空态']
  ],
  flow:'进入填写页后加载问卷题目和已有答案；用户完成输入并提交，系统依次校验访问资格、状态、频次和答案。成功后写入答卷并进入结果页；修改时定位原答卷并原位更新。',
  exceptions:'未开始、暂停、结束、删除、越权或超出提交次数时禁止提交；仅图片题选择非图片后清空；定位或签名演示取消时保持未填写；修改期间失去接收资格时拒绝保存。',
  data:'答案按“题目标题 + 答案值”兼容保存；多选展示时连接为可读文本，文件保存文件名，位置保存区域与可选详细地址。记录 response_open、response_submit、response_validation_failed、response_update；【假设】公共属性包含 survey_id、response_id、question_count、frequency、submit_date、timestamp，禁止上报答案正文。',
 risks:'正式环境依赖问卷访问鉴权、幂等提交、文件安全扫描、签名能力及定位授权。服务端必须防止重复提交并保留修改审计记录。',
 acceptance:'七种题型在 PC 与移动端展示和采集一致；必答、图片限制、日期时分和详细地址规则正确；单次、每日和不限次数控制正确；修改原答卷不增加提交数。'
 },
 responseDetail:{
 ...
 collection:{
  ...
  features:[
   ...
   ['收集方式','三种提交频次','支持仅提交一次、每日收集一次、不限次数提交；切换后规范化不适用字段。','P0','问卷新建与编辑共用'],
  ...
  acceptance:'教育局、学校的 PC 与移动端字段顺序和联动一致；三种收集方式来回切换无脏值；非每日方式不展示三项每日设置；范围、日期和提醒校验正确；发布后的未开始、进行中和已结束状态正确。'
 },
 templates:{
  ...
  features:[
   ['模板列表','筛选与分页','按名称和问卷类型筛选；PC 真实分页，移动端一次展示筛选结果。','P0','端内数据隔离'],
   ['系统模板','只读使用','系统模板仅提供“使用模板”，不允许原位编辑或删除。','P0','模板源数据不变'],
   ['自定义模板','新增与编辑','PC 按“基础信息、问卷设计”两步维护，第二步直接保存模板。','P0','同名校验'],
   ['模板应用','内容数据复用','PC 与移动端仅带入标题、类型、说明和题目；接收人员与收集设置使用新问卷默认值并由创建人重新配置。','P0','模板源数据不变'],
   ['移动选择','类型弹层','移动端问卷类型单选，无效类型自动回退全部。','P1','选择后刷新模板结果']
  ],
  flow:'用户在模板中心筛选并使用模板，或在“我的问卷”选择自定义创建、从模板创建。PC 维护模板时依次完成基础信息与问卷设计；使用模板后从问卷基础信息步骤开始，允许修改副本，模板源数据不变。',
  exceptions:'模板不存在、已删除、类型失效、同名或系统模板直接编辑时阻止保存；筛选无结果展示空态。',
  ...
  acceptance:'两端模板筛选与使用正确；自定义模板新增编辑仅 PC 可用，并仅展示两个步骤且在第二步保存；系统模板仅可使用；“我的问卷”可自定义创建或从模板创建；七种题型字段完整保存回填；移动端类型筛选状态正确。'
 },
 types:{
...
(() => {
  ...
  canCreateResponse = function (survey) {
    ...
    if (!records.length || survey.frequency === 'unlimited') return true;
...
function handleResponseAction(a,b){if(a==='select-my-response'){state.responseSelection[+b.dataset.surveyId]=b.dataset.responseId;renderShell();return true}if(a!=='submit-my-response')return false;const s=CONFIG[state.portal].surveys.find(x=>x.id===+b.dataset.surveyId);syncSurveyTemporalStatus(s);if(s.status!=='running'){toast(s.status==='not_started'?'问卷尚未开始':'当前问卷不可提交');return true}const answers=collectSurveyAnswers(s),answerError=responseAnswerError(s,answers);if(answerError){toast(answerError);return true}const records=MY_RESPONSES[state.portal][s.id]||(MY_RESPONSES[state.portal][s.id]=[]),personId=currentRecipientPersonId(),editing=b.dataset.responseId&&records.find(r=>r.id===b.dataset.responseId&&!r.external&&r.personId===personId);if(editing){if(!surveyAllowsCurrentPerson(s)){toast('您当前已不在问卷接收范围内');return true}editing.answers=answers;editing.updatedAt=responseTimestamp();state.responseSelection[s.id]=editing.id;location.hash=`list/response/${s.id}`;toast('答卷修改已保存');return true}if(!canCreateResponse(s)){toast(!surveyAllowsCurrentPerson(s)?'您当前已不在问卷接收范围内':'当前提交频次不允许再次提交');return true}const record={id:`${state.portal==='bureau'?'B':'S'}${s.id}-${Date.now()}`,surveyId:s.id,personId,submitter:currentUserName(),submittedAt:responseTimestamp(),valid:true,answers};records.push(record);s.fillStatus='submitted';if(s.internalDone!==undefined){s.internalDone+=1;s.done=submittedCount(s)}else s.done=(s.done||0)+1;syncDynamicSurveyRecipientTotal(s);state.responseSelection[s.id]=record.id;location.hash=`list/response/${s.id}`;toast('问卷提交成功');return true}
